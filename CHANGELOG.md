...
## 0.6.46 — le modele designe, le code ecrit

`dossier-copilote` iteration 2/4. Sur un indice que le linter signale deja comme manquant de detenteurs, l'auteur lance le copilote et **accepte ou refuse, un par un**, les personnages qu'il lui designe. Deux lots sequentiels, frontiere = **prefixe de chemin**, aucune fusion, **aucun `REJETE` du plan livre quand meme** (verifie sur le code, pas sur les comptes rendus).

- **Le modele ne rend que des JETONS, jamais un entier.** Le rang est une chaine `P1`..`PN` que l'assembleur emet, que le validateur constate par **appartenance**, et que le code re-resout par `Map.get`. Aucune conversion numerique nulle part — ni `Number`, ni `parseInt`, ni indexation arithmetique : ca supprime d'un coup la classe entiere des decalages base-0/base-1 **et** les cinq predicats numeriques (entier ? fini ? >= 1 ? <= N ? unique ?) qu'un ouvrier aurait inventes seul. La table des rangs **ne sort jamais** de `brain/copilote/` — KR-231 tenu par la **portee**, pas par une convention : zero occurrence des 14 symboles non exposes dans le code de la feature.
- **Le modele ne choisit PAS la certitude, et c'est une mesure qui l'a tranche.** `certitude` **ne decide de rien** dans `atteignabilite.ts` : un detenteur `croit` — une information **fausse** — eteindrait l'alerte `indice-sans-source` exactement comme un `sait` sincere. Le copilote aurait donc fait disparaitre **son propre declencheur** sans rendre l'indice plus obtenable. Le code pose `CERTITUDE_INITIALE`, meme autorite que l'editeur manuel. Le role qui avait contracte la certitude a **cede au tour 2** ; consequence en cascade non prevue : la sortie devient un tableau de chaines nues, deux predicats et un registre disparaissent, et l'invariant it1 « **zero cle commune** entre forme reseau et forme resolue » est **restaure**.
- **La liste vide est un SUCCES** — le predicat de non-vacuite de l'it1 **ne se transporte pas**. Il gardait une prose **scalaire**, ou le vide est une non-reponse ; sur une **liste**, le vide **est** une reponse. Punir la reponse honnete est une machine a complaisance : un modele qui ne peut pas dire « personne » nommera quelqu'un — et 4 personnages sur 6 du dossier de reference n'ont qu'**une** phrase d'audience `ia` pour les departager.
- **Une assertion verte deja au depot a redessine la moitie du plan.** `controles.test.ts:1143` disait depuis l'it1 que le seul indice signale de la fixture n'a **ni `verite` ni `formulation_joueur`**. Personne ne l'avait lue sous cet angle. Elle fait du **refus l'ecran nominal** de la fixture, promeut un cas limite en critere d'acceptation, et interdit le chemin passant sur la fixture — qui se **compose en memoire**, la fixture restant hors de tout lot. Chercher l'assertion existante qui contredit le plan coute moins cher que de la decouvrir a l'essaim.
- **Une contrainte d'outillage a dicte le contrat, et elle a ete mesuree AVANT d'etre tranchee.** `demander` est **surchargee sur le litteral de role** plutot que scindee en deux methodes — non par gout d'architecture, mais parce que les trois suites de la feature bouchonnent par `{ estDisponible, demander }` avec `demander` annote `jest.Mock` **nu** : un **membre** de plus rend ces litteraux incomplets, le lot contrat ne passe plus `tsc` **seul**, il happe des fichiers de feature, et le decoupage s'effondre. Une **surcharge** passe. Benefice : `demander('indice-detenteurs', d, {entiteId, champ})` est une erreur de **compilation** — et le `@ts-expect-error` qui le prouve a ete verifie **vivant** (TS2769 reel une fois retire), pas suppose.
- **La condition d'etat de `verite` est re-ecrite a ses DEUX sites** avec la 4e instance de l'instrument « present aux deux sites, mot pour mot ». Sans ca, les deux sites devenaient **faux** des cette iteration — et leur lecteur designe est l'ouvrier de la n°10, celui qui code la protection du **joueur**. Ce n'est **pas** une derogation d'audience (`verite` est deja `ia`) mais la levee d'une condition **temporelle** : `DEROGATIONS_AUDIENCE` reste vide et assertee vide. La clause qui l'autorise est **testable** — le schema de sortie du role ne porte aucune prose — et c'est elle qui empechera l'it3 de reutiliser cette porte pour un champ de texte.
- **Les deux plafonds deviennent des `Record` par role.** Un scalaire partage aurait **desserre la garde du role etroit par la mesure du role large, sans un seul test rouge**. Mesure : `M = 5361` (K sature) => budget **17 000** pour le nouveau role, **6000** inchange pour l'ancien ; `TAILLE_MAX_CORPS_IA` re-derive a **52 224** (pire cas 51 899, marge **325 o** — l'arrondi au kio **est** la marge). Le test de liaison passe en `describe.each`, le role le plus large **derive** par `reduce` et jamais ecrit : sur le role etroit, les canaris resteraient verts sans rien discriminer.
- **Le gabarit de sortie devient une table appariee au role**, et le canari qui compte est le **croise**. Deux constantes separees auraient laisse l'appariement role->gabarit **vivre uniquement dans le test**, qui en devenait un troisieme porteur. Le `Record` le rend **total a la compilation**. L'objection qui avait tue le `Record` au tour 1 — l'ancrage ligne-a-ligne du balayage — **se dissout** des que l'extraction s'ancre sur l'**entree** et non sur la declaration ; et ce balayage devait de toute facon etre reecrit, le fichier ne compilant plus une fois le budget devenu `Record`. **Deux roles a effort eleve ont echange leurs positions entre les tours** : l'arbitrage a tranche **sur l'argument, pas sur le mouvement**.
- **Six implementations fautives ecrites et vues ROUGES, puis reposees par la QA en mode B EN EDITANT LES VRAIS FICHIERS DE PRODUCTION** plutot que crues : gabarits intervertis (**7 rouges**, la panne se propage reellement a l'invite composee), un mot change d'**un seul** cote de la condition d'etat, garde `enVolRef` retire (**les deux** cartes rougissent), le gel retire (le `Select` retombe sur « Aucun indice signale » pendant l'acceptation), `@ts-expect-error` retire, et la marge du plafond re-mesuree a **325 o au chiffre pres**. Aucune sonde ne subsiste.
- **La frontiere des lots peut etre un PREFIXE DE CHEMIN, et c'est strictement mieux qu'une liste.** « Lot 1 ∌ `src/features/**`, lot 2 ⊂ `src/features/dossier-copilote/**` » se verifie d'un `grep`, par n'importe qui, sans relire 27 lignes. Zero diff sur les dix fichiers hors de tout lot ; `LigneProposition.tsx`, `cablage.test.ts` et `lintIsolation.test.ts` verts **sans une seule retouche**.
- **Ce que personne n'a verifie, ecrit sans euphemisme** : le **remontage du panneau a la navigation** est la **seule promesse du plan non tenue** — `estDisponible()` n'est pas reactif, le plan exigeait une **mesure** au lot 2, aucun test ne l'exerce et l'ouvrier signale honnetement ne pas l'avoir faite. Restent aussi hors de portee de tout instrument : la **qualite** de la designation et tout taux de complaisance (KR-229), la **justesse** du texte de la condition d'etat (seule sa presence aux deux sites est instrumentee), et le comportement reel du fournisseur amont (protocole **ratifie** par le comite, jamais eprouve en production).

- **La revue de PR a de nouveau trouve ce que les trois autres portes ne regardent pas**, et c'est la MEME famille qu'a l'it1 : un etat derive vif recalcule sur l'effet de bord de son propre geste. Le gel du § 3.6 couvrait le constat affiche et les options, **pas la valeur selectionnee** — apres une acceptation, le `Select` pouvait nommer une cible que la carte ne servait pas. Rien ne corrompait (l'ecriture lit la cible gelee), **l'ecran mentait pendant que l'auteur ratifiait**. Mais le motif de la revue etait a moitie faux, et la mesure l'a dit : le cas « a un seul constat, le champ s'affiche VIDE » **n'existe pas** — sans option correspondante React ne marque aucune `option` `selected` et un `<select>` de taille 1 retombe par spec sur la premiere, qui est l'option gelee. **Le premier temoin ecrit restait donc VERT sous le mutant** ; le defaut n'est atteignable qu'a DEUX constats, et c'est la que le temoin livre le pose. Une conclusion juste sur un motif faux cede au premier contradicteur (famille BUG-080) : les deux ont ete corriges. Cinq findings journalises, deux mutants ecrits et vus rouges.
- **Budget de contexte, releve dans ce meme lot — et DEUX plafonds franchis, compactes ici et pas au suivant.** `specification.json` de la feature a **69 253 o** pour 65 kio : les neuf arbitrages `[it1]` pesaient 7 704 o alors que leur code est livre et que leur revue porte deja le raisonnement — ils se reduisent a leur phrase d'arbitrage plus le renvoi, **66 079 o**, les 46 entrees conservees. Puis l'amendement de KR-236 (exige par la revue) a pousse `code-knowledge.json` a **77 593 o** pour 75 kio : resserrage des deux KR amendes, puis compaction de **KR-159** sur l'axe que le fichier nomme lui-meme — il recopiait trois decomptes d'une mesure ponctuelle du 2026-08-04, archivee dans la spec de `dossier-format` ; **76 621 o**. Et `bug_history.json` **se scinde** : BUG-097 a BUG-100 partent dans `bug_history.dossier-copilote.json`, leurs lecons etant portees par les entrees neuves qui les nomment — les garder les ferait relire deux fois. **9 510 o**. Restent a la corde et dits comme tels : le couple `CLAUDE.md` + `docs/WORKFLOW.md` a **28 octets** de son plafond, `code-knowledge.json` a 179.

- **Le second tour de revue n'a rien trouve dans le code — il a trouve le fichier TOUJOURS CHARGE.** `docs/WORKFLOW.md` § Worker Route Parity decrivait un worker de 111 lignes a UNE famille de routes et UN appelant, et annoncait la liste de controle des routes IA comme **restant a ecrire** : mesure, **424 lignes, DEUX familles, DEUX appelants, liste livree depuis l`it1**. Et sa **recette de releve manquait l'appelant IA** — le service compose son URL dans une variable, donc un gabarit ancre sur l'appel ne le voit pas. Un instrument de releve INVERSE est pire qu'aucun : il rend un vert qu'on cite. La lecon du lot : le balayage des documents perimes s'etait arrete **un fichier trop tot**, et juste avant le plus cher — celui qui est lu a chaque session, avant `code-knowledge.json`. Section reecrite sur l`etat mesure et **net-negative** (1168 -> 1030 o), la recette neuve **rejouee telle qu`ecrite**. Trois `minor` avec : une dette de focus **assumee et ecrite** plutot que corrigee en touchant une primitive partagee, la contrepartie du correctif majeur qui n`avait **aucun temoin** (son retrait laissait la suite verte en bloquant le Select pour toujours), et un renvoi circulaire ne de ma propre compaction. Neuf findings au total sur les deux tours, tous journalises.

## 0.6.45 — la premiere fois que le depot appelle un modele

`dossier-copilote` iteration 1/4. L'auteur ouvre la destination << Copilote >>, designe un personnage et un champ de prose, lance le copilote, puis **accepte ou refuse** le texte propose. Deux lots sequentiels, aucune fusion, **aucun `REJETE` du plan livre quand meme**.

- **Le worker entre dans la porte de commit.** Il n'avait AUCUN test (KR-233) et `testMatch` etait ancre sur `<rootDir>/src/**`. `jest.config.cjs` gagne `worker/**/*.test.ts`, `tsconfig.json` gagne `worker` dans `include` — **mesure d'abord** (`tsc` reste vert), puis **prouve capable d'echouer** : une sonde `const x: number = "une chaine"` fait rougir `TS2322`, fichier restaure avec SHA-256 identique. Un instrument qu'on n'a pas vu echouer ne mesure rien.
- **Le garde qui lie l'invite au schema porte sur le GABARIT, jamais sur la cle.** KR-236 : l'invite vit dans le worker, le validateur dans `brain/`, et leur divergence est la seule panne de cette tranche qu'aucune porte ne voit — deux appels, un refus, quatre textes corrects, suite verte, **pour toujours**. L'ouvrier a ajoute un troisieme cas negatif que le plan ne demandait pas, et c'est le plus instructif : une invite bavarde disant << la **valeur** de ce personnage >> satisfait `includes('valeur')` et **echoue** le garde. Aucun import `worker/` -> `src/brain/` (il trainerait du code client dans le paquet wrangler) : la liaison est un balayage de source a DOUBLE EXTRACTION, allow-list de deux porteurs, unicite d'occurrence assertee avant la comparaison.
- **Deux mesures justes se sont contredites, et aucun tour de comite ne l'a vu.** La QA a mesure un faux positif du scanner anti-identifiant (`objet.favori` dans de la prose saine) et propose d'exiger un chiffre ou un tiret dans le suffixe. Verification a l'arbitrage : ce correctif laisse echapper **`lieu.amorce`**, seme dans TOUT dossier cree. Sa premisse — << un identifiant contient toujours un tiret >> — est vraie de `randomToken()` et **fausse des identifiants semes**. Retenu : **forme LACHE intersectee avec `collectIds(dossier)`** — un identifiant est une appartenance au dossier, pas une silhouette. Trois mutants ecrits, dont celui-la ; la QA en mode B a repose le troisieme elle-meme.
- **Cinq mutants ecrits et verifies ROUGES, quatre REPOSES par la QA en mode B** plutot que crus : rejeu a 0 (4 tests rouges), rejeu illimite (le 3e bouchon est CONFORME, c'est lui qui separe), garde `enVolRef` retire (`Expected: 1, Received: 2`), branchement motif->texte inverse — que l'assertion d'unicite `Set(...).size === 4` **ne voit pas**, d'ou l'assertion par motif exigee en plus.
- **Les deux plafonds du meme appel, mesures dans le meme lot**, en deux grandeurs assumees differentes : budget client en **caracteres** (`6000`, refuse AVANT, il permet de nommer ce qu'il faut raccourcir), garde worker en **octets** (`19 456`, `TextEncoder`, refuse APRES). Le `body.length` du `PUT` existant compte des unites de code UTF-16 : **ni corrige, ni recopie**. Borne haute de 3 octets/unite **mesuree** (`'€'` = 3 pour 1 ; hors BMP = 4 pour 2). Canaris a ±1 des deux cotes, plus un discriminant de grandeur.
- **Une exception unique, bornee et non renouvelable** au critere n14 : quatre libelles promus dans `brain/dossier/libelles.ts`. La preuve de non-regression n'est pas un test neuf — c'est que les suites de `dossier-fiches` (132) et `dossier-canon` (46) restent vertes **sans une seule retouche**. Un diff dans leurs `tests/` aurait signale qu'une chaine a bouge.
- **Le fournisseur de modele n'etait nomme nulle part dans le plan** — manque de cadrage, pas d'execution, sur la premiere feature qui appelle un modele. L'ouvrier a **refuse d'inventer** une URL et un identifiant de modele : secrets d'environnement, `503 non-configure` tant qu'ils manquent. Le protocole ecrit (Anthropic Messages) est **son choix**, a confirmer avant deploiement. Ecrit sans euphemisme : `X-Sync-Key` est une cle d'espace de noms, **pas une autorisation** (KR-148) — quiconque en connait une valide peut bruler du budget modele, et aucun limiteur de debit n'est livre.
- **Deux retraits par leurs propres auteurs**, et ce sont les plus utiles. Le PM a retire sa coupe de la variante REMPLACEMENT apres mesure : aucun mecanisme du depot ne marque un champ de personnage, donc la coupe laissait un **etat non dessine** ou le texte de l'auteur disparaissait sans trace. Et le Narratif a retire son propre motif << le bloc AVANT est le SEUL garde anti-complaisance >> en le qualifiant de **surestimation de couverture** — le diff attrape l'auto-paraphrase de la cible, jamais la paraphrase du contexte, qui reste non couverte et part dans la revue.
- **La revue de PR a trouve deux defauts que personne d'autre n'avait regardes**, et c'est le meilleur argument pour cette porte-la. (1) `valeurAvant` etait **recalcule a chaque rendu** depuis un abonnement qui se reveille sur `dossier:updated` — c'est-a-dire sur **l'effet de bord de l'acceptation elle-meme**. Resultat : un bloc << AVANT >> **apparaissait de nulle part** sur une cible vide, et sur une cible redigee le texte de l'auteur **disparaissait de l'ecran a l'instant ou il etait ecrase** — exactement l'etat non dessine qui avait fait retirer la coupe de REMPLACEMENT. Tous les temoins existants s'arretaient **avant** l'acceptation ; aucun ne regardait le rendu d'apres. `valeurAvant` se **gele sur le geste** desormais. (2) `code-knowledge.json` prescrivait encore, dans KR-236, **le garde que le comite venait de rejeter** — et c'est le seul fichier lu en entier avant d'ecrire du code : il l'aurait enseigne au lot contrat de l'it2, qui pose precisement une deuxieme invite. **Regle qui en sort** : quand un raffinage MESURE qu'un instrument decrit dans un KR est faux, l'amendement du KR part dans le MEME lot que le code. Huit `minor` corriges aussi, dont un **cas negatif creux** qui n'executait jamais la comparaison qu'il pretendait eprouver (KR-235 applique au garde cense proteger de ce mode de panne). 10 findings, 4 portes au journal, `97 suites / 1388 tests`.
- **Budget de contexte, releve dans ce meme lot.** `code-knowledge.json` etait a **+42 o** de son plafond. KR-235 y gagne le correctif faux de (2) — sans quoi l'entree **orientait le lecteur vers le piege** — KR-236 son amendement, et les deux se resserrent dans le meme geste : **76 758 -> 76 792 o**, marge **+8**. `bug_history.json` franchit son plafond a la revue de PR et se **SCINDE** : BUG-085 et BUG-086 partent dans `bug_history.dossier-controles.json`, leurs lecons etant promues ailleurs (KR-199, la skill `raffinage-iteration`, et `docs/WORKFLOW.md`) ; BUG-074 **reste**, son correctif n'etant toujours pas fait. Et son `_about` se compacte sur **son propre axe** : il etait devenu le journal de toutes les scissions passees alors qu'il ecrit lui-meme que leur historique vit ici — **2 589 -> 1 428 caracteres**, l'axe et l'etat courant restent, l'historique part la ou il est deja lu. **11 292 -> 10 126 o**, marge **+114**. Les autres tiennent : `features_history` +510, roadmap +149, `specification.json` de la feature +20 057.

## 0.6.44 — le copilote propose, le code ecrit

Cadrage de la feature n°8 `dossier-copilote`, la **derniere du Temps 1** et la **premiere du depot a appeler un modele**. Comite a 5 roles, deux tours, aucun veto survivant. Aucune ligne de `.ts` : ce lot est une tranche d'outillage + le plan.

- **Quatre croisements de position, tous motives, tous au tour 2** — et c'est pour cela que l'arbitrage n'est pas un decompte de voix. Sur `destinations.ts`, le Tech Lead et le Narratif ont **echange** leurs positions : le Narratif a cede en cassant d'abord la sienne, apres avoir mesure que son propre << toujours charge >> du tour 1 commencait par un champ d'audience `auteur`. Motif qui tranche, apporte par celui qui cede : **l'asymetrie du regret** — une proposition appauvrie se voit le jour meme, un champ `moteur` entre dans un contexte de redaction ne se voit jamais.
- **Garde d'audience STRICTE, zero derogation.** Consequence assumee plutot qu'exception : `canon.objectifs[]` porte sept cles et **zero `ia`**, donc << Eclater le synopsis >> v1 ne recoit pas les objectifs et propose des personnages sans les rattacher. **On retrecit l'assistant, on n'ouvre pas la table** — une liste de derogations a trois entrees en a quatre dans six mois.
- **Trois instruments proposes en comite, trois mesures, deux faux.** `FORME_IDENTIFIANT` est **ancree** : un balayage anti-identifiant ecrit avec elle rend toujours faux sur une sortie serialisee — **inerte, vert quoi que le modele produise**. Une version non ancree matche `fin.tout` et `objet.favori` dans de la prose saine (`fin`, `lieu`, `objet`, `indice`, `climat`, `jalon`, `objectif` sont des mots francais courants). Et le garde a seuil de 12 caracteres est aveugle aux nombres — donc a la totalite du perimetre qu'il pretendait couvrir — et fait rougir du code sain : **une garde qui apprend a modifier le temoin est pire que pas de garde**. La regle survit, l'instrument est a reecrire.
- **Le SS 2.8 reformule en une regle PARAMETREE** plutot qu'en deux regles a tenir en phase : le modele ne cree jamais, il rend un brouillon **sans identite** dont le code frappe l'identifiant a l'instant ou l'auteur accepte ; l'unique parametre est l'ensemble des formes de brouillon admises — ferme et non vide en redaction, **VIDE en jeu**, et c'est ce vide qui donne << l'IA ne peut referencer que des identifiants existants >>.
- **3 -> 4 iterations**, ordre impose par l'echelle de risque **rangs -> nombres -> entites** : prose seule, indices, fiche complete, **synopsis en dernier**. Le Tech Lead avait retire cette quatrieme tranche sur un decompte de voix deja perime alors que son motif de fond disait l'inverse. Cout chiffre et assume par le PM : le synopsis vaut **de l'ordre du quart** de la valeur — le plus spectaculaire, pas le plus utilise.
- **Budget de contexte, releve dans ce meme lot.** `code-knowledge.json` etait a **+656 o** de son plafond depuis `dossier-controles` it10, avec consigne de ne plus rien recevoir. Compacte sur un axe ecrit dans son `_about` — *une entree qui recopie une mecanique de jeu, un registre de donnees, ou un paragraphe d'un fichier deja charge a chaque session n'est pas un invariant, c'est un doublon qui derive* — : **77 456 -> 70 726 o** sur 29 entrees, **puis 8 KR neufs** (KR-229..236) ramenent a **76 758 o**, sous le plafond de **42 octets seulement**. `docs/ROADMAP-BASCULE-IA.md` franchi par les memes edits : les cinq trous **clos** du SS 5 et un bloc clos du SS 1 en sortent (archive, jamais la colonne `Statut`) — **36 311 -> 35 691 o**.
...
- **L iteration a ete DECOUPEE EN TROIS avant d etre raffinee**, et le critere de coupe n est pas le volume mais le **sens d erreur**. Saturer les ARETES est monotone et sur : chaque severite nouvelle est vraie. Fermer une PORTE DE RACINE — un savoir sans porte, un jalon sans declencheur — **RETIRE** un producteur, et c est la que vivent les faux positifs. Les melanger aurait mis une transformation demontrablement correcte dans le meme lot qu une transformation a hypotheses.
- **Le comite a affirme le pouvoir discriminant d un test sans l executer — deux fois.** Le temoin cense prouver << c est un point fixe, pas une passe >> ne prouvait ni cela ni la non-sur-propagation : la QA en mode B a pose les deux implementations fautives sur le code livre, **les 7 tests sont restes verts**. Une chaine a deux aretes se sature en une passe (le maillon intermediaire entre dans le `Set` sans jamais etre depile, et la reconstruction ne teste que l appartenance FINALE) ; et dans la chaine rompue le maillon orphelin est **isole**, donc une propagation non orientee n a rien a lui propager. Corrige dans le meme lot — quatre maillons, plus un temoin d arete **reliee** — et les deux mutants font desormais rouge. BUG-087.
- **La regle de la skill etait appliquee a moitie.** << Une affirmation sur la couleur d un test se MESURE >> avait ete tenue sur les **valeurs attendues** — mesurees, justes, confirmees a l unite. Elle ne l avait pas ete sur le **pouvoir separateur du temoin**, qui est une autre verification : << ce test rend 1/1/1 >> s execute sur le code BON, << ce test rougirait sur une mono-passe >> ne s execute que sur le code FAUTE.
- **Deux messages, une remediation — sur une asymetrie mesuree dans le code, pas sur un gout.** Le message est grave dans le constat a l emission, donc deux messages coutent zero champ ; la remediation est resolue plus tard depuis `constat.niveau` seul, donc deux consignes auraient exige un discriminant sur une interface **exportee par le baril** : un etat illegal representable. Trois roles sur quatre avaient converge sur << un seul texte >> en appliquant au message une demonstration qui ne vaut que de la remediation.
- **Le plan etait internement contradictoire, et aucune porte ne l a vu.** << Sans champ neuf >> + << signature inchangee >> + << lire deux index >> ne peut pas etre satisfait ensemble. L ouvrier l a **signale plutot que de le contourner**, et resolu par l ensemble de cles de la carte — cle absente, cle presente a tableau vide, cle pleine : trois etats, epingles par un test qui sait echouer.
- **Et le comite a mandate une phrase que sa propre demonstration ne couvrait pas.** Le message disait a l auteur que ses enchainements << bouclent >> ; la condition d emission n etablit que << des aretes, aucune racine >>. Remonter les amonts dans un graphe fini tombe sur un cycle **ou** sur une chaine simplement non racinee — et la seconde est **le cas le plus courant** : l auteur chaine `A -> B -> C` et oublie de raciner `A`. Meme faute que celle que le comite avait lui-meme refusee sous la variante 22. Trouve en revue de PR, corrige, et surtout **rendu mecanique** : l absence de cycle est desormais LUE SUR LA DONNEE dans le temoin, pas promise en prose. BUG-088.
- **Les deux TEMPS dans un lot unique.** T1 le deplacement seul, constate vert avec **une seule** assertion touchee, PUIS T2 le comportement. C est l unique instrument qui rend << deplacement, jamais reecriture >> relisible en diff — une reecriture sous couvert de deplacement ne se voit dans aucun diff.

## 0.6.38 — le rapport voit ce que seul le validateur voyait

`dossier-controles` iteration 5/6. Les DIX emplacements ou `validateDossier` produit deja un avertissement entrent au rapport, par UNE entree de registre qui lit `warnings` et une table DECLAREE de dix lignes. Zero ligne de code de production de feature.

- **La valeur n'est pas celle qu'on annoncait.** Le cadrage defendait « l'auteur le voit sans reimporter ». Mesure : les dix sites ont DEJA une affordance sur leur propre ecran — quatre panneaux lisent `validateDossier().warnings` en ligne, le compteur du canon rougit seul. Mais **sept sur dix ne se montrent que pour l'entite AFFICHEE** : savoir si l'un de douze personnages a une contre-mesure non armee demandait d'ouvrir douze fiches. Le gain est **l'agregation et la portee**, pas la revelation.
- **Une garde interdisait plus que son propre commentaire ne motivait.** Le test d'it3 interdisait a `controles.ts` d'importer le validateur ENTIER, au nom d'un motif qui ne porte que sur le canal `error` (un dossier persiste n'en porte jamais, KR-225). Or `warnings` est l'inverse exact : le seul canal dont tout l'interet est qu'il **survit** a la persistance. La garde retrecit sur son motif — `.errors` interdit, `.warnings` exige, **les deux moities**, sans quoi elle passerait a vide.
- **Une seconde garde encodait une coincidence.** Le predicat « racine du chemin differente de la section » est **structurellement insatisfiable** pour la section `canon` : `sections.ts` en fait la seule des dix dont la cle n'a pas de point. Il tenait parce qu'aucune des cinq regles livrees ne s'enracine dans `canon` — pas parce qu'il etait vrai. **Il n'a PAS ete relache** : le temoin retenu le satisfait deja, donc relacher aurait coute de la couverture sur cinq regles pour n'acheter rien. A la place, l'invariant lui-meme est desormais tenu par un balayage de source qui interdit tout decoupage de chemin, `canon` compris (KR-226).
- **Deux affirmations du comite sont tombees sur des chaines executees.** « Le message du validateur est deja une phrase redigee » : faux sur cinq sites, qui rendent litteralement un nom de champ JSON dans la prose de l'auteur. « La consigne est verifiee mot pour mot » : elle dit d'ajouter une condition structuree la ou il manque une **duree**, et **contredit son propre message affiche deux lignes plus haut**. Aucune des deux n'a ete arretee par le raisonnement.
- **La proposition la plus seduisante a ete rejetee par la mesure.** Declarer les dix `location` paraissait renforcer la table ; mesure faite, `validate.ts` les produit avec la **meme** fonction que le rapport importe deja — elle rend « Personnage « Selene la Vigie » », pas « Personnages ». Declarer aurait remplace un nom d'entite resolu par un seau ecrit a la main.
- **Le cout est chiffre, et le facteur n'est pas ce qui decide.** `validateDossier` coute plusieurs dizaines de fois `controlerDossier`, pour ~1 a 2 ms par rendu d'ecran : negligeable en absolu, enorme en facteur. Les deux s'ecrivent. Le cache reste refuse — **premier rejet de l'iteration qui repose sur un nombre** et non sur une doctrine. Trois entrees de registre — une par code — auraient coute six validations par rendu contre deux : d'ou UNE entree.

## 0.6.37 — le rapport cesse d'etre une impasse

`dossier-controles` iteration 4/6. Chaque ligne du rapport devient un bouton qui **mene a la section ou le constat a ete produit**, et qui **dit ou elle mene avant qu'on clique** — un trailing avec le titre de la section, derive du registre, jamais ecrit a la main.

- **Le motif du trailing n'est pas celui qu'on croyait.** Il a d'abord ete defendu comme une garde d'honnetete — empecher un clic de contredire la prose. Mesure faite : `QUOI FAIRE` est de la prose **redigee regle par regle**, pas derivee de `controle.section`, et **5 constats sur 10** du dossier de reference ne nomment jamais leur section (les cinq `personnage-sans-voix`, qui disent « Caractere exploitable », un bloc de fiche). Sur un dossier neuf : **0 sur 4**. Le trailing est donc le seul element de la ligne **mecaniquement garanti juste ET complet**.
- **Le vrai risque n'etait aucune des quatre tensions du cadrage** : `panneauControles.test.tsx` epinglait **l'inverse exact** de l'iteration — « aucune ligne n'est un arret de tabulation ». L'iteration ne completait pas cette suite, elle **renversait un invariant pose expres**. Renverse **nommement, dans le meme `describe`**, jamais supprime : c'etait la seule garde de forme de la ligne.
- **« Combien de tests rougissent » etait la mauvaise question.** Mesure par experience controlee : sur un type mesapparie entre fichiers, **`jest` reste vert a 35/35 pendant que `tsc` rougit sur trois fichiers** — dont celui qui *consomme* la prop. La porte reelle d'un changement de signature est le compilateur.
- **L'invariant de BUG-082 est desormais tenu par le TYPE, pas par une consigne.** Le panneau recoit un `SectionId`, jamais l'union locale : il **ne peut pas exprimer** la destination « Controles », donc ne peut pas fabriquer les deux lignes courantes qui avaient coute BUG-082.
- **Une seule expression par ligne alimente l'affichage ET l'action**, ce qui rend la coherence vraie **par construction** : un libelle ne peut pas mentir sur sa destination. Deux roles s'etaient **cede mutuellement** ce point ; c'est la position argumentee sur le fond qui l'a emporte, pas la politesse du dernier qui cede.
- **Trois affirmations fausses ont traverse le comite**, toutes presentees comme mesurees, deux venant de l'orchestrateur : « aucune regle ne mene a son propre remede » (c'est 9 sur 10), le rayon de souffle chiffre en tests, et la conversion `<p>` en `<span>` donnee pour forcee (React ne reparente rien, une seule assertion bascule). **Aucune n'a ete arretee par le raisonnement** — les trois l'ont ete parce que deux roles sont alles lire le registre et executer des sondes.
- 1 lot unique, six fichiers, **sans marque `contrat`**. L'argument qui a ferme le debat du decoupage est **textuel** : la decision qui *autorise* de toucher `DossierEditorScreen.tsx` est conditionnee a « un lot unique possedant ce fichier en exclusivite » — decouper l'aurait annulee. **85 suites / 1218 tests verts** (avant : 85 / 1214).
## 0.6.36 — l'auteur voit ce qui, dans ses registres, rend l'aventure injouable

`dossier-controles` itération 3/6. Quatre entrées de registre de plus — un indice qu'aucune source ne produit (**bloquant**) ou qu'une seule produit (**alerte**), un lieu de départ sans personne (**bloquant**), un personnage qu'on ne peut rencontrer nulle part (**alerte**), un personnage sans voix propre (**info**). Aucune surface neuve : it1 et it2 avaient livré le rendu générique exprès.

- **« Cinq règles » en était quatre.** « Indice orphelin » et « goulot d'étranglement » sont **une cause à deux seuils sur un seul compteur**, pas deux causes — tech-lead et narratif y arrivent séparément. La formulation concurrente, à deux branches disjointes, laissait `indice.cendres-tiedes` (zéro savoir, un seul delta) **tomber entre les branches** : le linter se serait tu sur un indice à source unique, c'est-à-dire sur le cas même qu'il existe pour attraper.
- **« Un indice que personne ne détient » était mesurément faux, et c'était écrit dans la phrase de démo.** Détenir n'est qu'une des **six** familles de production (savoirs, les quatre sites d'effets `reveler_indice`, et l'enchaînement `mene_a`). Une règle lue depuis les seuls savoirs aurait déclaré **bloquant**, à son premier run, un indice de `dossier-minimal.json` — la fixture de toutes les preuves de la feature. Corrigé jusque dans le `goal` : « qu'**aucune source ne produit** ».
- **La clause d'audience héritée d'it1 était fausse** — `savoirs[].indice_id` est rangé `ia`, et c'est la seule ligne `ia` de toute la table portant un `…_id`. **Amendée là où elle était écrite**, jamais glosée ailleurs : le critère n'est pas l'audience de la clé lue, c'est la **nature du geste qui éteint le voyant** — rédiger de la prose pour le modèle n'est jamais bloquant, poser une référence entre deux entités existantes peut l'être.
- **Un prédicat universel sur l'ensemble vide est vrai** : « lieu de départ désert », écrite naïvement, se déclenchait **par vacuité sur tout dossier neuf**. Seule des cinq règles à porter sur une *collection* et non sur ses éléments, elle porte **trois gardes** — les quatre autres sont des filtres et se taisent d'elles-mêmes.
- **Les deux défauts les plus chers n'étaient pas dans le code qu'on modifiait, et n'ont surgi qu'au second tour** : un balayage de `controles.test.ts` exigeait que *chaque* règle du registre parle sur un dossier fraîchement semé — ce que les règles neuves ne peuvent structurellement pas faire ; et un **littéral de test d'une autre feature** suffisait à casser une assertion. Le balayage est refondu en `Record<ControleId, Dossier>` **total par compilation** : toute règle future est non livrable tant que son auteur n'a pas exhibé un dossier qui la déclenche.
- **Un accord unanime sur une impossibilité n'est pas une mesure.** Orchestrateur et tech-lead avaient conclu que « `mene_a` à plat contre saturé » n'était pas prouvable sous la discipline du clone muté d'un seul champ ; la QA a trouvé la mutation — **remplacer la collection** plutôt que d'y ajouter une arête. Le choix porte donc un test, et la saturation d'it6 le fera basculer délibérément.
- **La QA en mode B a trouvé deux affirmations du plan optimistes dans le sens qui surestime la couverture** (même direction dangereuse qu'à it2) : un cas limite annoncé couvert sans l'être, et l'absence de `severity` tenue par le seul typage pour les règles neuves. Corrigées **en écrivant les tests manquants**, puis **sondées** — un `severity` injecté dynamiquement passe `tsc` et rougit bien le balayage.
- **Budget de contexte** : trois plafonds franchis dans ce lot, trois traitements différents et conformes. `specification.json` **compacté** (71 034 → 65 507 o) — 14 décisions d'it1 et it2, code livré et revue écrite, réduites à leur phrase d'arbitrage plus le renvoi ; aucune décision perdue, 38 avant, 38 après. `ROADMAP-BASCULE-IA.md` **compacté** (36 355 → 35 384 o) en renvoyant deux historiques de recadrage vers les specs — c'est un index, pas un journal. `bug_history.json` **scindé** et non compacté, comme sa règle l'exige — mais sur le bon axe, trouvé en revue de PR après une première tentative erronée. L'axe de ce fichier n'est pas « de quelle feature vient l'entrée » mais « sa leçon est-elle **déjà promue** dans un instrument en lecture obligatoire » : BUG-082, BUG-083 et BUG-084 partent donc dans un `bug_history.dossier-controles.json` neuf — les deux premiers sont **nommés dans la skill `raffinage-iteration`**, qui porte leurs parades en section dédiée ; le troisième renvoie à KR-199 par sa propre mitigation. **BUG-074 reste**, pour la deuxième fois explicitement : son correctif n'est pas fait et sa leçon n'est promue nulle part — « l'archiver serait l'oublier » est écrit dans le fichier, et c'est ce que la première tentative avait manqué.
- 1 lot unique `contrat`, aucun essaim : zéro composant, zéro prop, zéro export neuf — rien à paralléliser. **85 suites / 1214 tests verts** (avant : 85 / 1213).

## 0.6.35 — la navigation cesse d'attendre qu'on ouvre le rapport

`dossier-controles` itération 2/4. Chaque ligne de la navigation porte désormais le pire niveau de contrôle de sa section, **fondu dans le badge de compte existant** : Départ porte `BLOQUANT`, et Personnages portera `3 fiches · BLOQUANT` à partir d'it3. L'auteur voit qu'une section cloche **pendant qu'il en édite une autre**, ce que le panneau Contrôles seul ne pouvait pas lui offrir.

- **« Fusionné au badge de compte » était INCONSTRUCTIBLE tel que le cadrage l'écrivait**, et la mesure est nette : les deux sections que les règles d'it1 allument (`canon`, `depart`) sont **exactement** les deux dont le compte vaut `—`. Une concaténation mécanique aurait affiché « — · BLOQUANT » — un badge dont la moitié gauche ne dit rien — **au premier rendu de production**, pas dans un cas limite. D'où la **règle d'élision** : le mot **remplace** le tiret, il ne s'y ajoute jamais.
- **L'instrument qui aurait tout prouvé par la couleur est CASSÉ, pas absent** — mesuré trois fois, dont deux fois hors du rôle qui l'a trouvé. `toHaveStyle({ color: 'var(--bad)' })` **passe sur un `Badge tone="muted"`** : le CSSOM de jsdom rejette les jetons `var()` sur `color`, les deux côtés du matcher retombent sur la chaîne vide, et l'assertion est verte quelle que soit la teinte. Conséquence portée par toute l'itération : **le ton se prouve au contrat** (une fonction pure qui rend `'bad'`), **jamais au rendu** ; la vue est gardée par une sonde de source. Au passage, cela invalide une « faisabilité vérifiée » invoquée au raffinage : le test cité passait parce qu'il passe toujours.
- **Le mapping mot + teinte existait déjà**, privé de `ListeControles.tsx`. Une copie dans `bascule-editeur` aurait été **invisible au lint** — c'est une copie, pas un import — les deux suites seraient restées vertes, et elle aurait divergé au premier renommage. Extrait en `brain/dossier/pastilles.ts`, **deux fonctions pures, aucun composant partagé** : les deux surfaces partagent la *décision*, pas le *balisage*. Le lot contrat emporte donc la migration de son premier consommateur — sans elle, la sonde de porteur unique ne pouvait pas être verte à sa propre porte.
- **Le comité s'est corrigé lui-même sur les trois désaccords structurants, et chaque retrait est venu de l'auteur de la position** : le tech-lead a retiré **sa** substitution (KR-218 dit « toujours fusionné ») **et son propre** composant partagé (il aurait exigé une prop `compte`, c'est-à-dire son propre veto) ; la QA a retiré **sa** variante « teinte seule » sur deux motifs de son terrain — `info → muted` *est* la teinte du calme, donc le critère aurait été **faux** et non mal écrit.
- **Deux rôles se sont croisés sur la prop `niveauxParSection`** : le tech-lead l'a concédée pendant que la QA retirait son exigence et proposait un mock. Cas que le rituel ne prévoit pas — les notes de tour 2 s'écrivent sans se voir, donc la convergence peut se manquer. Tranchée : la prop, **requise jamais optionnelle** (« deux chemins dont le testé n'est pas le livré, c'est pire que pas de prop du tout »).
- 2 lots séquentiels, aucun worktree. **85 suites / 1202 tests verts** (avant : 83 / 1193). Aucun changement de forme sur `RapportControles` : le lot contrat est une **extraction de présentation**, jamais un contrat de schéma.
- **BUG-084, et c'est la leçon de l'itération** : **deux** affirmations du plan sur le comportement d'un test étaient fausses, et toutes deux avaient été *raisonnées* au lieu d'être *exécutées*. « La sonde de teinte serait restée verte » — elle aurait rougi, son assertion **positive** sur `tone="muted"` ne survivant pas au remplacement du littéral ; et « les états 4, 6 et 7 sont prouvés au composant » — seul le 4 l'est. Le premier motif avait été posé au tour 1, **maintenu** au tour 2, recopié deux fois dans le plan et avait traversé la porte mécanique. La QA en mode B les a trouvés en **rejouant l'ancien test contre le fichier livré**. *Règle qui en sort : tout plan qui écrit « ce test resterait vert » rejoue le test avant de l'écrire — le coût est un run.*
- Raffinage complet (2 tours, 4 rôles, aucune `ESCALADE`, 15 désaccords tous statués) : `.claude/raffinage/dossier-controles-it2.plan.md`, revue : `.claude/raffinage/dossier-controles-it2.revue.md`.

## 0.6.34 — l'éditeur cesse de se taire sur ce qui n'a pas été écrit

`dossier-controles` itération 1/4 — **le squelette du linter d'aventure**. Un écran « Contrôles » s'ajoute sous les dix sections et dit à l'auteur ce qui, dans son dossier, n'a pas encore été rédigé. La première règle porte sur les quatre proses semées à la création : **bloquante** sur le texte d'ouverture, que le moteur lirait au joueur mot pour mot, **alerte** sur les trois proses du canon, qui partiraient telles quelles dans le contexte du modèle.

- **La signature figée au cadrage était CIRCULAIRE, et c'est le compilateur qui l'a dit — pas une relecture.** `ControleDescripteur.controler()` rendait des `Controle` dont l'`id` est `keyof typeof CONTROLES` : le type du registre dépendait de son propre initialiseur. Cassée par `ConstatControle` (tout sauf l'`id`) puis `Controle extends ConstatControle`. _Une signature écrite dans un document de cadrage n'est pas exécutable tant qu'aucun rôle ne l'a passée à `tsc`._
- **`niveau` et `section` vivent sur le CONSTAT, pas sur le descripteur — et le motif décisif n'est ni it1 ni it3, mais it4.** Le narratif proposait deux entrées de registre (`amorce-ouverture`/`amorce-canon`, KR-164 « un code par cause ») ; il les a retirées en constatant que la spec déclare déjà « objectif sans chemin » comme _une_ règle à deux niveaux. Le tech-lead a obtenu les deux champs sur le constat en mesurant que « référence orpheline » sera **une** cause atteignant cinq sections — une `section` typée sur le descripteur la transformerait en cinq entrées pour une cause, KR-164 en sens inverse. Contrepartie livrée : `ControleDescripteur.niveaux` déclare l'ensemble fermé émissible, balayé par un test.
- **`includes`, jamais `startsWith`** — la QA défendait la convention d'`amorce.test.ts` et l'a retirée devant l'argument qui tranche : ce test porte sur le **semeur**, qui maîtrise la tête de sa chaîne, tandis que le linter lit un texte **édité par l'auteur**, qui ne la maîtrise pas. `startsWith` éteindrait le voyant sur une prose rédigée _autour_ du glyphe — le cas exact que le dispositif existe pour attraper. Le test « marqueur au milieu » est le seul qui falsifie les deux comportements.
- **Un SECOND landmark `<nav aria-label="Contrôles">`, frère du premier** — sortie hors des deux options débattues. L'ordre de tabulation suivant le DOM et non le landmark, l'UX garde sa onzième position au clavier ; le test des dix lignes reste vert **sans modification** (rendu conditionné à l'injection du panneau) ; et le landmark « Sections du dossier » cesse de mentir, « Contrôles » n'étant pas une section.
- **`jouable` est calculé, exporté, testé — jamais affiché.** Tout dossier neuf naît injouable : un bandeau rouge s'allumerait sur 100 % des dossiers à t=0, miroir exact du voyant tautologiquement vert déjà refusé sous `SANS_COMPTE`. Refusé d'avance avec lui : toute exemption « dossier jamais édité », dont le seul discriminant serait `updatedAt === createdAt`, faux dès la première édition sans rapport.
- **Le glyphe `⟨à écrire⟩` reste interdit en source dans toute la feature**, et c'est une conséquence de **conception**, pas d'hygiène : la garde d'`amorce.test.ts` balaie tout `src/`, donc `ListeControles` reçoit ses contrôles en props et son test les fabrique ; seul `PanneauControles` lit le dossier réel (KR-223). Piège évité de justesse côté contrat : écrire « `controles.ts` importe le marqueur mais ne le ré-exporte pas » **en commentaire dans le baril** aurait fait rougir la seconde garde, qui asserte sur le texte brut.
- 2 lots séquentiels (contrat puis écran — `App.tsx` importe le panneau, donc aucun parallélisme n'était possible), aucun worktree. **83 suites / 1193 tests verts.** Aucun lot contrat sur `types.ts`/`destinations.ts`/`validate.ts` : cette feature n'ouvre aucune tranche de schéma.
- **Deux défauts trouvés par la QA en mode B, corrigés dans ce lot.** **BUG-082** (major) : l'écran rendait **deux lignes courantes simultanément** — deux `useState` indépendants, donc deux sources pour « ce qui est affiché ». Or cette forme avait été **nommément rejetée par le tech-lead au tour 1**, dans l'annexe de sa note — et la version condensée que l'orchestrateur a écrite dans le dossier de raffinage avait **perdu cette ligne** : l'ouvrier ne pouvait pas la lire, et la QA n'a pas pu retrouver la prémisse qu'on lui citait. _Règle qui en sort : un `REJETÉ` d'annexe se recopie dans le registre des désaccords du plan — un refus qui ne franchit pas ce registre n'existe pas pour l'essaim._ **BUG-083** (minor) : `toHaveTextContent` est une **inclusion**, pas une égalité — l'assertion « aucun badge de compte » serait restée verte si un `trailing` avait été ajouté (cinquième occurrence de KR-199).
- Raffinage complet (2 tours, 5 rôles, aucune `ESCALADE`, 13 désaccords tous statués) : `.claude/raffinage/dossier-controles-it1.plan.md`, revue : `.claude/raffinage/dossier-controles-it1.revue.md`.
- **Budget de contexte — deux franchissements traités dans ce lot.** `code-knowledge.json` a franchi 75 kio en accueillant KR-217 à KR-225 → **compacté** : `KR-023`/`KR-025`/`KR-068` (modèle d'arbre, feature en sommeil, démolition n° 9) réduits à leur invariant plus un renvoi à leur spec ; `KR-151`/`KR-152` réduits à **une ligne nommant la règle ESLint** qui les porte désormais ; `KR-153` renvoyé à `docs/WORKFLOW.md`, où sa procédure vit déjà. `bug_history.json` a franchi 10 kio → **scindé** (il ne se compacte pas) : BUG-079/080/081 partent dans `bug_history.dossier-registres.json`, leur feature étant close et ses leçons promues en KR. **BUG-074 reste** dans le fichier vivant, son correctif n'étant toujours pas fait.

## 0.6.33 — plancher de session

Aucun changement fonctionnel. Bump de plancher de session (`docs/WORKFLOW.md`, § Versionnement) posé avec le **cadrage de `dossier-controles`** (n° 7) : `specification.json` de la feature — 4 itérations, 10 règles (5 bloquantes, 4 alertes, 1 info) au lieu des 8 du plan-cible — et répercussion du § 2 / § 5 de `docs/ROADMAP-BASCULE-IA.md`, resserré sous son plafond de contexte dans le même geste. Détail des quatre corrections mesurées : l'entrée n° 7 du roadmap et les `resolved_decisions` de la spec.

## 0.6.32 — un climat cesse d'être un nom seul et gagne la phrase que le narrateur lira

`dossier-registres` itération 5/5 — **la feature est terminée**. L'auteur tient le registre des climats de son aventure : un nom, une durée en pas d'horloge, et une **manifestation** — ce que le narrateur lit tant que ce climat est actif. La section « Conditions » cesse d'être le dernier état vide de l'éditeur : les **dix** sections du dossier ont désormais un vrai formulaire, et la dernière racine du schéma est refermée.

- **Le motif écrit au cadrage contre un éditeur d'effets sur Climat était FAUX, et c'est l'UX — seule contre quatre — qui l'a démontré.** `KR-208` n'interdit qu'une opération à opérande entier _neuve_, pas l'usage des quatre existantes ; le tech-lead et `narratif-ia` l'ont concédé au tour 2. La conclusion (aucun `EditeurEffets` sur `FicheClimat`) tient, mais sur deux motifs trouvés seulement alors : un climat n'a **aucun instant d'application** (ni `declencheur_texte` ni `declencheur_expr`, `climat_actif` étant un état de session) et **aucune idempotence** (rien n'enregistre « déjà appliqué ») — et le besoin exprimé est déjà servi par un Événement, qui a un instant daté. _Un refus juste pour un motif faux reste un refus fragile._
- **`Climat.duree` est un ENTIER, jamais de la prose** : la collision de nom avec `plan_actions[].duree` (KR-198) **s'éteint** au lieu d'être contournée — même mot, même sens, un compte de pas d'horloge. L'extinction d'un climat garde ainsi un propriétaire de code, et l'entier est borné gratuitement par la boucle générique `POSEURS_D_ENTIER` là où la prose n'aurait eu aucun instrument.
- **`Climat.manifestation`, audience `ia`, bornée à 20 mots** — unique proposition `INNOVATION` de l'itération, portée par `narratif-ia`, acceptée par le PM contre l'engagement « rien d'autre n'entre ». Sans elle, `monde.conditions.climat[]` serait la **seule collection de registre du schéma à zéro champ `ia`** : la n° 10 aurait injecté le libellé nu et fait improviser au modèle, à chaque tour, du lore que l'auteur n'a pas écrit. Un contrat d'injection est déposé avec elle, opposable à la n° 10 : un seul climat par tour, `manifestation` seule injectée, absente ⇒ le bloc est **omis**, jamais paraphrasé.
- **Premier raffinage de la feature à 5 rôles** — `narratif-ia` convoqué parce que la question centrale était une question d'audience sur la frontière moteur. Il a fourni le motif réel du refus ci-dessus **et** la seule innovation : le critère de convocation mérite d'être élargi à toute itération qui pose ou déplace une ligne de `destinations.ts`.
- Une preuve d'**absence** ne s'appuie jamais sur la liste de fichiers d'un lot (arbitrage QA, contre une formulation que deux rôles avaient signée) : ce n'est pas un instrument rejouable après le merge. Le critère a été scindé en deux niveaux, plus un troisième sur la non-corruption d'un `effets_regles` non vide.
- 2 lots séquentiels (contrat — deux champs, deux lignes de table, deux destinations, une dispense — puis écran, zéro fichier `brain/`), aucun worktree. **81 suites / 1173 tests verts.** Aucun ricochet sur les fixtures partagées : ni un entier ni une prose libre ne référence l'espace `pnj`. QA mode B : 8/8 critères vérifiés, dont 4 sondes de discriminance réellement exécutées puis restaurées par sauvegarde de fichier.
- Reporté nommément plutôt que passé sous silence : le **§ 09 du plan-cible (« effets chiffrés sur les règles ») reste NON TENU** à la fin du Temps 1 — `Climat.effets_regles` est un `Delta[]` que rien ne peut remplir, et `docs/REGLES-DU-JEU.md` ne contient aucune section sur les modificateurs ambiants. La consommation réelle de `duree` par la n° 14 n'est vérifiée par personne.
- Raffinage complet (2 tours, aucune `ESCALADE`, 7 désaccords tous statués) : `.claude/raffinage/dossier-registres-it5.plan.md`, revue : `.claude/raffinage/dossier-registres-it5.revue.md`.
- **Budget de contexte — trois franchissements traités dans ce lot** (jamais reportés au suivant). `dossier-registres/specification.json` a franchi 65 kio → **compacté** : les `goal` des cinq itérations livrées réduits à leur phrase d'arbitrage + le renvoi à leur revue, qui porte déjà le raisonnement. `features_history.json` et `bug_history.json` ont franchi 10 kio → **scindés** (ces deux-là ne se compactent pas) : `dossier-fiches` et `dossier-objets` partent dans `features_history.0.6.x-anterieures.json`, et BUG-066/069/071 dans `bug_history.dossier-fiches.json`, leur feature étant terminée et archivée. **BUG-074 reste dans le fichier vivant malgré la scission** : son correctif n'est pas fait et sa leçon n'est promue nulle part — l'archiver serait l'oublier. À surveiller : `code-knowledge.json` est à **76 797 o pour un plafond de 76 800** — trois octets de marge, la prochaine itération qui ajoute un KR déclenchera sa compaction.
- Revue de PR tech-lead : `REQUEST_CHANGES` puis `APPROVE`, 2 majeurs et 5 mineurs corrigés avant que la tranche n'atteigne l'auteur (BUG-079 à BUG-081). Le plus instructif — **BUG-080** : le motif que cette itération démontre faux survivait mot pour mot dans deux fichiers `brain/` que le lot possédait, alors que la correction avait été portée à la spec et au type. Même classe que BUG-069, reproduite dans le lot même qui en fait sa leçon — d'où le durcissement : on ne corrige plus « les endroits auxquels on pense », on **grep la formule renversée** et on traite la liste.
...
- **`DELTAS` (le registre des 4 effets de règle) n'était exporté d'aucun fichier hors `brain/dossier/`**, gardé par un test qui l'interdisait explicitement — même famille de trou que `MARQUEUR_A_ECRIRE` en it2 (KR-214). Résolu par un export nommé depuis `brain/index.ts` et une réécriture du garde en allow-list nommée (deux porteurs exacts : `brain/index.ts`, `EditeurEffets.tsx`, construits avec `path.join` pour tenir sous Windows) — jamais une projection dérivée qui aurait contourné le contrat (veto tech-lead). KR-215.
- **`EtapeQuete` ne porte aucun champ `etape`** — proposé au tour 1 avec renumérotation gardée, retiré par le tech-lead lui-même au tour 2 : un ordinal persisté serait de l'état dérivé miroité dans la SSOT (KR-013), et son propre précédent (`plan_actions[].etape`, `dossier-fiches`) désynchronise déjà après un retrait. L'ordre est celui du tableau, l'étiquette « ÉTAPE N » est recalculée à chaque rendu.
- **Nommage du champ prose de la quête, en deux temps** — `objectif` écarté d'entrée (KR-198, mot proscrit ailleurs dans le schéma), `enonce` proposé puis retiré par le tech-lead lui-même au tour 2 (collision avec `charpente.jalons[].enonce_texte` déjà existant), `consigne` retenu — audience `ia` (le contenu d'une quête doit atteindre le modèle narrateur pour être jouable).
- **`EditeurEffets` générique sur `DELTAS[].refKinds.length`**, jamais fixé à l'arité 1 observée aujourd'hui : une `Select` CIBLE par entrée de `refKinds`, ajout à deux temps (rien n'est écrit tant que l'effet n'est pas bien formé), l'EFFET d'une ligne déjà écrite n'est jamais muté (retirer + réajouter).
- 2 lots séquentiels (contrat minimal — 3 lignes de registre, export `DELTAS` — puis écran, zéro fichier `brain/`), aucun worktree. 79 suites / 1145 tests verts. Un fichier hors-lot (`dossier-format/tests/importDossier.test.tsx`, feature déjà terminée) a dû être mis à jour en aparté : un compte d'anomalies en dur augmente mécaniquement d'une unité, le nouveau `Quete.donneur_id` référençant l'unique personnage de la fixture minimale — 3ᵉ occurrence de ce même ricochet, désormais tracée en `open_questions` pour le raffinage futur.
- Raffinage complet (2 tours, aucune `ESCALADE`, un veto tech-lead sur `DELTAS` résolu par export nommé, un nom de champ retourné en cours de route par son propre auteur) : `.claude/raffinage/dossier-registres-it3.plan.md`, revue : `.claude/raffinage/dossier-registres-it3.revue.md`.
...
- **`caractere.curseurs` suit la même forme que `stats` (jurisprudence it3) — optionnel en bloc, TOTAL quand présent — sans en partager le mécanisme.** Un curseur manquant ne rend aucun jet irrésoluble (contrairement à `stats`) : la ressemblance de forme n'a jamais justifié une abstraction commune. `brain/dossier/curseurs.ts` (nouveau registre : `CURSEURS`, `CURSEUR_VALUES`, `CURSEUR_MIN`/`MAX`, `PARLER_REPLIQUES`) **n'importe rien** de `characteristics`/`challenge`/`combat`/`xp` — l'`affinite` (CA/IN/IG) est une union locale, jamais `Characteristic` : donnée de présentation, hors score de mutation et hors table dorée (KR-193), vérifié aussi bien par test-grep que par lecture directe en QA mode B.
- **`PARLER_REPLIQUES = 2` est une borne d'INTERFACE, jamais une règle `validate.ts`.** Convergence indépendante des trois rôles (tech-lead, UX, QA) au raffinage : le bouton d'ajout disparaît au plafond (jamais `disabled`), et un document importé qui en porte davantage se rend intégralement, sans troncature ni refus.
- **`cede_si` (« ce qui le fait céder ») est le SECOND champ du schéma dont l'injection dépend du RÔLE qui demande** (après `Relation.secret`, it5) — patron `Relation.secret` : injecté au seul acteur du personnage porteur, exclu du narrateur, des autres personnages et de l'arbitre, sans dépendre d'un fait de session (contrairement à `plan_actions[].si_bloque`). Prédicat écrit mot pour mot à deux sites (JSDoc de `Caractere.cede_si` + commentaire de sa ligne dans `destinations.ts`) — et, découverte du raffinage : **l'identité des deux sites est en réalité testable automatiquement**, `couverture.test.ts` portant déjà cet instrument pour `Relation.secret` depuis it5. Le plan la classait à tort « non vérifiable en l'état » ; le lot contrat a écrit le test.
- **Dernière tranche de schéma de la feature : la sonde discriminante de `couverture.test.ts` qui suivait la prochaine tranche non instanciée n'a plus de cible interne.** Rebasée en négatif de forme de chemin (précédent déjà en place pour `plan`/`plan_actions`), jamais supprimée — vérifiée rouge avant, verte après.
- 2 lots séquentiels (contrat `brain/dossier/`, puis feature `dossier-fiches/`), racines de chemin disjointes — aucun worktree, intégration réduite à un contrôle de propriété. 73 suites / 1052 tests, tous verts. Score de mutation **non requis** (aucun des 4 fichiers de règles touché — `curseurs.ts` est un registre de présentation, KR-193/KR-161).
- **La feature `dossier-fiches` (n° 4) est terminée : 8/8 itérations.** Ses trois entrées de `features_history.json` antérieures à cette clôture ont été archivées dans `features_history.0.6.x-anterieures.json` (leurs leçons sont déjà des KR de `code-knowledge.json`) pour faire de la place à la sienne — 1ʳᵉ scission de `features_history.json`, 5ᵉ scission de fichier append-only du dépôt (les 4 précédentes : `bug_history.json`).
...
- **Redécoupage en DEUX temps de l'ancienne it5 (savoirs+relations+présence+retrait), un plan-fleuve à 4 têtes.** D'abord retrait → it7 (avant le tour 1 du comité). Puis, sur MESURE du tech-lead au tour 1 : le hook d'écriture, déjà à 690 lignes, aurait atteint ~1295 lignes (1,6× le plafond bloquant KR-112) avec les 3 familles restantes — **savoirs coûte ZÉRO ligne de contrat** (déjà typé et destiné depuis `dossier-format`, seul l'écran manque), donc c'est lui qui part, dans une it6 propre et légère. La feature passe de 6 à **8** itérations, confirmé par l'humain aux deux étapes du rétrécissement.
- **`presence.quand` tranché `auteur`**, précédent `but.echeance` (it4) **a fortiori** : une échéance anticipée fait tomber une horloge en avance ; une **disponibilité** lue par le narrateur la ferait **contredire la scène que le moteur vient d'assembler** — « il n'est là que la nuit » narré alors que le moteur l'a placé ici à midi. `relations.intensite` tranché **`moteur`** — même doctrine que les 8 caractéristiques et les 6 curseurs (KR-193) : un nombre signé qui code un fait de jeu, ni brut ni paraphrasé avant qu'un libellé dérivé par le code n'existe. `INTENSITE_MIN`/`INTENSITE_MAX` (-3/3) sont des constantes **distinctes** de `CONFIANCE_MIN`/`CONFIANCE_MAX` bien que leurs valeurs coïncident aujourd'hui — précédent `CAMPS_PERSONNAGE`/`CAMPS`, `PorteeContreMesure`/`Portee`.
- **Le mécanisme de gating de `relations[].secret` est écrit en un prédicat exact, mot pour mot à deux sites** (JSDoc + commentaire `destinations.ts`) : une ligne de relation n'entre que dans le contexte de l'acteur du personnage qui la **PORTE** ; si secrète, jamais dans celui de la cible ni du narrateur ; `secret: false` élargit d'**un** rôle (le narrateur), jamais de tous. Zéro code d'assemblage en it5 — aucun assembleur n'existe avant n°10/n°12, seule la présence textuelle du prédicat est vérifiable (un test-grep l'automatise, ajout du lot contrat au-delà de ce que le plan demandait, jugé légitime en QA : cohérence documentaire, pas comportement).
- **Bug réel trouvé en raffinage, pas en code review** : `brain/components/Stepper.tsx` affichait « +-2 » pour une valeur négative avec un préfixe signé. L'UX a corrigé le seuil (`value > 0`, pas `value >= 0` comme le tech-lead l'avait d'abord écrit) — à zéro, une intensité neutre n'affiche ni « +0 » ni signe. Premier appelant réel de `prefix` signé dans le dépôt ; `StepperProps` inchangée.
- **Dette KR-112 du hook fermée : `hooks/useEcriturePersonnages.ts` (690 l.) scindé en 4 sous-hooks par sous-domaine** (socle, identité, plan, relations+présence) + un assembleur. Le socle porte seul `commit()` et ses deux indexations KR-197 — jamais recopiées. Les deux tests KR-197 existants (identité, caractéristiques) restent verts **sans une ligne d'assertion modifiée**, la preuve que la scission est correcte. Deux fichiers repassent au-dessus du signal 400 (`useEcriturePlan.ts` 415 l., `FichePersonnage.tsx` 417 l., loin du bloqueur 800) — 3ᵉ itération consécutive où la dette se déplace plutôt que se résorbe, datée pour it6/it7/it8.
- **Blocage hors lot, conséquence directe du contrat** : la relation auto-référentielle ajoutée à la fixture minimale a fait passer un test pré-existant de `dossier-format` de 3 à 4 anomalies attendues — corrigé (2 lignes), même classe que BUG-071 (it4).
- **Écart accepté** : `PanneauPersonnages.tsx` touché (+3 lignes additives) malgré un plan qui le classait deux fois hors lot — un lot qui étend les props consommées par un composant enfant doit câbler son site d'appel unique, même hors périmètre visuel nominal.
- **Revue de PR : un vrai bug fonctionnel.** L'état vide « aucun autre personnage » de `BlocRelations.tsx` masquait des relations DÉJÀ ÉCRITES — un dossier à un seul personnage portant une relation auto-référentielle (légale depuis ce lot, instanciée dans la fixture minimale) rendait ses champs invisibles et inéditables, violant le critère racine #11 de la feature. Corrigé (`personnages.length < 2 && relations.length === 0`) avec son test.
- 69 suites / 993 tests. Score de mutation **non requis** (aucun des 4 fichiers mutés touché).

## 0.6.21 — un personnage cesse d'être un nom sur une liste et devient un acteur avec un but

`dossier-fiches` itération 4/6. L'auteur donne à son personnage un **but**, un **plan d'actions** par étapes (avec durée et porte de sortie), et — s'il en fait un antagoniste — des **contre-mesures**, dans le 4ᵉ bloc de l'accordéon, « Objectif & plan d'actions ».

- **`but.echeance` tranché `auteur`, jamais `ia` — désaccord SWAP entre deux rôles, chacun sans voir la position finale de l'autre.** tech-lead ouvrait au tour 1 sur `auteur` (asymétrie du regret : aucun consommateur avant la n° 14, `auteur → ia` se desserre sans coût, l'inverse est impossible sur un `schema: 1` sans migration), puis passait à `ia` au tour 2 en lisant l'argument de narratif-ia (cohérence : `libelle`/`pourquoi` sont déjà `ia`) — pendant que narratif-ia, au même tour, revenait à `auteur` en lisant sa propre doctrine (« une échéance en prose reste une donnée d'horloge », précédent `declencheur_texte` : le narrateur ne doit pas provoquer/improviser ce que le moteur n'a pas constaté). Deux positions échangées **simultanément et indépendamment**, arbitré `auteur` par l'orchestrateur au tour 3.
- **`plan_actions[].duree` et `contre_mesures[].delai` : `moteur`, entier, `DUREE_MIN=1`.** `si_bloque` : `ia`, injecté **seulement** si le moteur déclare l'étape bloquée — didascalie, aucun jumeau `_expr`, aucune entrée dans `FAMILLES_DE_CONDITIONS` (précédent `cede_si`), contrôle de cohérence isolé et propre dans `validate.ts`.
- **Aucun repli de LECTURE sur `duree` — trouvé et corrigé en revue de PR.** `BlocPlanActions.tsx` affichait `etape.duree ?? DUREE_MIN` : un Stepper qui fabrique et **committe** silencieusement une valeur `moteur` que l'auteur n'a jamais posée, la même classe que l'aurait fait `stats ?? STATS_INITIALES` en lecture (interdit depuis it3). Corrigé par l'affordance pointillée « + Poser une durée… », qui committe `DUREE_MIN` au clic ; le `Stepper` ne monte qu'une fois la valeur posée.
- **6e famille de conditions D1 matérialisée, contrastée à celle qui reste calme.** `contre_mesures[].declencheur_texte` sans `_expr` avertit (7e ligne de `FAMILLES_DE_CONDITIONS`, `alerteSansExpr: true`) ; `plan_actions[].declencheur_texte` sans `_expr` reste délibérément calme (arbitrage d'it1). Le critère racine #7 de la spec, qui exigeait l'avertissement sur les deux familles, était **inatteignable tel qu'écrit** — trouvé indépendamment par tech-lead et QA au tour 1, scindé en 7a/7b.
- **Trou résiduel de BUG-050 fermé (BUG-070) : les listes OPTIONNELLES à éléments structurés n'étaient jamais validées.** `LISTES_A_ELEMENTS_STRUCTURES` ne dérivait que de `LISTES_REQUISES` — un `contre_mesures: ["une chaîne"]` traversait `validateDossier` en `ok:true`. `LISTES_OPTIONNELLES_STRUCTUREES` ajoutée ; pertinent pour it5 (`relations[]`, `presence[]`, deux listes optionnelles de plus).
- **Le critère #8 (dossier de référence sans régression + liste malformée refusée) était prouvé en deux moitiés, sur deux documents différents — jamais ensemble.** Trouvé par la QA en mode B sur un lot déjà vert. Fermé par un test qui corrompt `monde.personnages[1].contre_mesures[0]` **dans** `dossier-reference.json` (6 personnages) et vérifie, dans le même résultat, que SEULE cette anomalie remonte.
- **Dette KR-112 datée par it3 (désaccord n° 17) fermée : deux extractions, pas une.** `hooks/useEcriturePersonnages.ts` soulage `PanneauPersonnages.tsx` (466 → 184 l.) ; `BlocCaracteristiques.tsx` extrait le bloc 3 pour que le bloc 4 neuf ne fasse pas repasser `FichePersonnage.tsx` au-dessus de 400 lignes (442 → 397 l.). Le hook lui-même atteint 690 lignes — au-dessus du signal de scission, sous le seuil bloquant : la dette s'est **déplacée plutôt que résorbée**, à traiter par un futur lot d'extraction.
- **Deux blocages mécaniques, hors des deux lots, tous deux conséquences directes d'un mandat du plan.** `pvDerive.test.ts` (discriminant KR-169) rougissait après l'extraction de `BlocCaracteristiques.tsx` — retargeté. Le test combiné fermant le critère #8 — ajouté après le tour QA.
- 67 suites / 964 tests. Score de mutation **non requis** (aucun des 4 fichiers mutés touché).

## 0.6.20 — un personnage cesse d'être un portrait et devient jouable

`dossier-fiches` itération 3/6. L'auteur règle les **8 caractéristiques** de son personnage dans le 3ᵉ bloc de l'accordéon, et voit son **PV** — qui n'est ni saisi, ni choisi, ni stocké — se recalculer sous ses yeux.

- **`Personnage.stats` : optionnel EN BLOC, TOTAL quand présent.** Jamais `Partial`. Un personnage sans bloc n'est pas mécaniquement jouable ; un personnage avec bloc a **les 8 clés**, donc tout jet est résoluble en **un seul** point de contrôle. Les deux rôles qui portaient chaque camp ont **échangé leurs positions entre les deux tours** du comité (2ᵉ fois dans cette feature) : le tech-lead a mesuré son propre argument faux et est passé au `Partial`, pendant que narratif, UX et QA passaient au TOTAL. Ce qui a tranché n'était sur le terrain d'aucun des deux — un bloc partiel crée un état où le moteur **ne peut pas résoudre un jet**, et sa troisième issue est « laisser le modèle improviser » ; et sur un `schema: 1` **sans chemin de migration**, desserrer TOTAL→Partial est gratuit quand resserrer Partial→TOTAL est impossible. On choisit la contrainte qu'on peut encore relâcher.
- **Le « total » ne coûte AUCUNE ligne à `validate.ts`.** `sitesDe` coupe sur `if (!estObjet(site.valeur))` : bloc absent = zéro site = **calme**, bloc à 1-7 clés = 7 sites `undefined` refusés par les 8 lignes `requis: true` d'`ENUMERES_FERMES`. Le typage et l'exécution disent la même chose sans qu'on l'ait écrit deux fois — une entrée de schéma reste **une ligne de table**, pas une branche de code.
- **L'état vide EST le mécanisme.** « + Régler les caractéristiques… » écrit les 8 clés à `CARACTERISTIQUE_MIN` en **un** commit : le passage de « non réglé » à « réglé » est un **geste explicite** de l'auteur, jamais l'effet de bord du premier stepper touché. C'est ce qui rend tenable la ligne rouge du tech-lead (« ne pas fabriquer 7 valeurs `moteur` que l'auteur n'a pas posées ») **sous** une forme totale : l'alternative — un repli de **lecture** `stats ?? STATS_INITIALES` — afficherait huit `1` et un PV `3` au montage d'un personnage jamais touché, exactement la paire de valeurs que BUG-064 interdit de semer.
- **`?? STATS_INITIALES` : légitime à l'ÉCRITURE, interdit à la LECTURE.** Exporté depuis `brain/`, il est à un import du moteur du Temps 2, où un repli de lecture ferait résoudre un jet contre une fiche **fabriquée que personne n'a écrite**. Tenu par une docstring **et** par une assertion du test structurel `pvDerive.test.ts`.
- **Le PV est dérivé, jamais stocké** (KR-192) : `maxPV({FO,AG,EN})` appelé **en ligne**, aucune clé `pv` au document. Un test-grep interdit la somme écrite à la main sous `src/features/**` et `src/player/**`, avec l'assertion discriminante qui l'empêche d'être vert par vacuité — sans elle, un grep qui ne trouve rien passe pour une preuve.
- **Les 8 clés sont destination `moteur`, une ligne par caractéristique.** Le chiffre est un **seuil** (réussite = dés ≤ carac) : un modèle qui lit `FO: 9` connaît la marge avant que `challenge.ts` n'ait résolu, et racontera l'issue avant le tirage. Ni le nombre, ni le PV, ni une **paraphrase** (« FO élevée ») ne franchissent la frontière tant que la n° 10 n'a pas livré un libellé dérivé **par le code** et sa ligne d'audience. Ce qui remplace le chiffre côté prose existe déjà et est écrit par l'auteur : `apparence` et `fonction`, dont le JSDoc dit « elle DÉCRIT, elle ne chiffre pas ».
- **`docs/REGLES-DU-JEU.md` corrigé EN PREMIER, dans le même lot que le code.** Le plancher `1` n'était écrit dans aucune section — le § 1 ne donnait que `2D4` (min 2) et « plafond 10 à la création », qui sont une **procédure de génération du héros**, pas la borne de l'échelle. KR-130 impose doc → code ; et une raison **mécanique** s'ajoute au principe : `docs/WORKFLOW.md` fait sauter les deux revues sur un changement `.md` seul, donc livrée à part, la phrase qui devient la source d'une borne de schéma serait la seule ligne de l'itération que **personne ne relit**.
- **`CARACTERISTIQUE_MIN` va dans `dossier/types.ts`, pas dans `characteristics.ts`** — fichier sous score de mutation : y toucher déclencherait le run **et** le cliquet `break` 80 → 85, qu'une mesure à 81,40 % ne paie pas pour une constante d'une ligne. **Limite écrite, pas découverte** : l'échelle vit désormais à deux endroits (MAX là-bas, MIN ici) jusqu'à la n° 9. Mesure de sortie : `git diff --stat -- src/brain/characteristics.ts` rend vide, `break` reste à **80**.
- **BUG-068 — deux tests promettaient dans leur nom plus que leurs assertions ne prouvaient.** Un `jest.spyOn` posé **après** le rendu qu'il observe (syntaxiquement présent, sémantiquement mort — c'est sa voisine qui tenait le critère), et « les 8 Stepper » prouvé sur **3**, l'échantillon **recouvrant** le PV, ce qui laissait DX/IN/IG/SE sans aucune preuve de lecture. Trouvés par la QA en mode B sur un lot déjà vert. **3ᵉ occurrence de la classe** après BUG-064 et BUG-065 → frappée en **KR-199**. Détail qui vaut d'être gardé : prouver que c'est le **spy** qui rougit, et non l'assertion voisine, a demandé d'inverser temporairement l'ordre des deux — sans quoi la sonde validait la voisine et laissait la garde nommée sans preuve.
- **Trois plafonds de budget de contexte franchis, compactés dans le même lot.** `bug_history.json` (6 o de marge) inaugure un **5ᵉ axe de scission** : une entrée dont la mitigation est déjà **promue** dans un instrument lui-même en lecture obligatoire est relue à chaque session **sous sa forme promue** — la garder fait payer la même leçon deux fois. `code-knowledge.json` rend le corps de KR-095 à la spec de `cloud-sync`. Le `specification.json` de la feature réduit ses décisions **it1 et it2 livrées** à leur phrase d'arbitrage + le renvoi au dossier de raffinage : la revue est le dossier, la spec en est l'index.
- **Aucune extension de `Stepper`.** Proposée puis retirée par son autrice : sous le schéma total, `Stepper` n'est jamais monté quand `stats` est absent — il ne reçoit jamais `undefined`. Un état vide bien placé a coûté moins qu'un composant élargi.
- 67 suites / 940 tests. Score de mutation **non requis** (aucun des 4 fichiers mutés touché).

## 0.6.19 — les gardes d'isolation cessent de dépendre d'une liste tenue à la main

Lot d'outillage, hors cycle de feature. Convergence portée depuis `plan-global.md` § 3 de projetx (colonnes genliv). Aucun changement de comportement produit : rien de ce que l'auteur voit ne bouge.

- **`FEATURE_DIRS` codé en dur → liste dérivée du disque.** `.eslintrc.cjs` lisait un tableau de neuf noms recopiés à la main ; une feature créée sans y être ajoutée était **silencieusement exemptée** des deux règles d'isolation — ni erreur, ni avertissement. La parade en place était un test de **liste** (`featureDirs.test.ts`, `readdirSync ⊆ FEATURE_DIRS`) : il gardait un littéral, pas un comportement. `readdirSync` dans la config fait disparaître la classe de panne **par construction** — il n'y a plus de liste à tenir à jour, donc plus de dérive possible.
- **Un bloc `overrides` par feature, au lieu d'un seul sur `src/features/**`.** La feature X n'interdit plus que les **autres** : le faux positif documenté en commentaire depuis l'origine (« un fichier de X qui s'importe lui-même par un chemin grimpant `../../X/…` serait signalé — KNOWN LIMIT, not fixed ») **disparaît** au lieu d'être toléré.
- **Le sens `brain/ → features/` est enfin tenu.** `CLAUDE.md` l'exigeait depuis toujours (« Feature-agnostic — never imports from features/ ») et **rien ne le vérifiait**. La règle arrive **verte** : zéro violation mesurée avant de la poser — elle interdit une régression, elle ne rattrape pas une dette. C'est la moitié manquante de l'isolation, portée depuis chrono-sabine.
- **`featureDirs.test.ts` → `lintIsolation.test.ts`.** Ce qui reste à prouver n'est plus « la liste est juste » mais « les règles **mordent** » : le test lance le **vrai** ESLint sur la **vraie** config du dépôt, et **chaque refus est accompagné de son discriminant** — un extrait voisin qui doit passer. Sans discriminant, un test de refus reste vert le jour où la règle refuse tout. 22 sondes.
- **BUG-027 corrigé** (ouvert depuis outillage-1, `minor`, zéro occurrence). Le sélecteur `TemplateElement` était ancré en tête de quasi : `` `#abc123` `` était refusé, `` `background: #abc123` `` **passait**. Il cherche désormais partout dans le quasi, avec un lookbehind borné qui porte l'équivalent du **masquage `var(...)`** de projetx — une couleur écrite en **repli** de token n'est pas une couleur en dur. Les quatre sondes que la fiche du bug prescrivait sont écrites, y compris celles qui doivent **passer**.
- ⚠ **Une limite connue est épinglée par un test plutôt que découverte plus tard** : `` `Section #123` `` reste un **faux positif** — `#123` est indiscernable d'un hexadécimal à 3 chiffres par une expression régulière seule. projetx s'en protège en ne vérifiant que l'intérieur des props `sx`/`style` ; la règle de genliv porte sur tout le dépôt, donc plus large et plus exposée. Le test **rougira** le jour où quelqu'un corrigera, en signalant que le test et le commentaire de config se mettent à jour ensemble. Le masquage à parenthèses équilibrées de projetx exigerait une fonction de règle, donc le paquet `eslint-plugin-local-rules` — **non installé pour zéro bénéfice présent** : 0 repli `var()` et 0 `color-mix()` mesurés dans le dépôt.
- **`stryker.config.json` → `stryker.config.mjs`.** Une seule raison : JSON ne porte pas de commentaire, donc la **rationale d'inclusion/exclusion par fichier** n'avait nulle part où vivre — elle se transmettait de session en session par la mémoire de quelqu'un. Les quatre fichiers mutés portent maintenant chacun le motif de leur inclusion, et **les exclusions portent le motif de leur exclusion** (registres tenus par la table dorée, `dossier/` tenu par `couverture.test.ts`, présentation et écrans sans arithmétique de jeu). **Aucune option n'a changé de valeur** — seule la clé `$schema` disparaît (le `@type` JSDoc la remplace en éditeur ; `stryker.config.mjs` n'est ni dans `tsconfig.include` ni linté, donc plus rien ne le vérifie mécaniquement). Les deux réglages à ne jamais « corriger » (`tempDirName` sans point, `cleanTempDir`) gardent leur justification. Auto-découverte du `.mjs` **vérifiée** dans `SUPPORTED_CONFIG_FILE_NAMES` de `@stryker-mutator/core`, pas supposée.
- **La cadence à l'itération était déjà en place** (`docs/WORKFLOW.md`) mais ne vivait que là. Elle est désormais écrite dans le fichier que le `dev-contrat` ouvre, avec la ligne `git diff --stat` qui décide mécaniquement si le run est requis — « modifié », jamais « appelé ».
- **`resolved_decisions` : rien à adopter, la prémisse du plan est périmée pour genliv.** Mesuré : **6 features sur 9** en portent déjà (jusqu'à 44 entrées pour `dossier-format`), options rejetées et arbitrages motivés compris. Les 3 sans (`book-library`, `cloud-sync`, `play-mode`) sont exactement les features **dormantes ou repointées** par la bascule, dont aucune n'est en cours. Rien de fabriqué pour cocher une case.
- **Aucune dépendance ajoutée.** Les types d'ESLint sont déclarés localement (`src/types/eslint.d.ts`, la surface exacte utilisée : trois membres) plutôt qu'installés — règle « No unnecessary dependencies ».
- 65 suites / 923 tests. Score de mutation sans objet (aucun des 4 fichiers mutés touché).

## 0.6.18 — un personnage a un visage avant d'avoir des chiffres

`dossier-fiches` itération 2/6. L'auteur écrit l'identité de son personnage — sa fonction, son apparence, ce que le joueur en sait — dans le 2ᵉ bloc de l'accordéon. Et l'écriture cesse d'être muette : un dossier disparu sous ses pieds le lui dit désormais, sous la fiche du personnage en cause.

- **Redécoupage : la feature passe de 5 à 6 itérations.** L'it2 cadrée disait « l'identité **et** les 8 caractéristiques » : phrase de démo avec « et », ≥ 9 critères, et un lot contrat portant deux tranches de schéma disjointes (trois proses libres + huit bornes numériques) en un seul passage. Scindée avant le comité, pas après. Les caractéristiques + le PV dérivé deviennent it3 ; les anciennes it3/it4/it5 glissent d'un rang.
- **`Personnage` étendu** : `fonction?`, `apparence?`, `description_joueur?`, trois `string` optionnels **à plat** (aucun sous-objet `fiche{}`), destination `'ia'` — même arbitrage que les trois proses de `monde.lieux[]` : le narrateur du Temps 2 doit les **lire** pour incarner le personnage, ce ne sont pas des notes de rédaction.
- **Le suffixe `_joueur` désigne l'AUDIENCE, jamais le RÉGIME** — écrit noir sur blanc dans `destinations.ts`, parce que les deux régimes coexistent déjà sous ce suffixe : `canon.partage.accroche_joueur` est `ia`, `charpente.depart.texte_ouverture_joueur` est `moteur` et reste la **seule** prose émise mot pour mot. `description_joueur` est injectée, jamais restituée verbatim.
- **`commit()` ne rend plus `void`** — livré **avant** le premier champ de prose libre, ordre imposé par le plan : la dette était datée « premier geste du lot d'it2 » depuis la revue de PR d'it1. `statut:'absent'` (dossier supprimé dans un autre onglet) n'est plus avalé en silence. Le bandeau branche sur `statut`, jamais sur `issues` : `{statut:'absent'}` ne porte **ni** `errors` **ni** `warnings`, et un `IssueList` vide aurait rendu un bandeau vide sous un eyebrow rouge.
- **BUG-065 — les deux gardes de KR-197 étaient justes et prouvées par rien.** Le seul test allumant le bandeau ne semait qu'**un** personnage : il serait resté vert avec les deux gardes supprimées, porte verte et compte rendu revendiquant l'inverse. 4ᵉ occurrence de la classe après BUG-056/061/063, **première attrapée avant d'atteindre `main`** — par la QA en mode B, pas par la porte. Aucun changement de code ; deux tests nommés, un par indexation, chacun discriminant sous **sa propre** sonde avec contrôle croisé. Règle qui en sort : une propriété à deux moitiés s'écrit en **deux** critères, ou porte « prouvé séparément » dans son libellé.
- **Le budget de mots sur les proses de personnage : proposé, puis retiré par son auteur.** `destinations.ts` porte déjà **huit** chemins de prose `ia` sans borne, dont deux — `canon.ton`, `canon.interdits_ton[]` — **toujours chargés**, donc plus lourds par tour que n'importe quelle fiche. En borner trois sur onze ferait cesser le silence de l'instrument de signifier « sous budget ». Reporté en **un seul balayage** : n° 10 pour les nombres, n° 7 pour le rendu. Rien d'irréversible n'est perdu à attendre — un budget est un avertissement non bloquant, il ne touche pas `schema: 1`.
- **Le pari du plan sur KR-190 s'est vérifié à la mesure** : une tranche de prose pure ne touche **ni** `tables.ts`, **ni** `validate.ts`, ni leurs tests. KR-190 borne le pire cas, il ne prescrit pas la liste — un plan qui recopie « les 8 fichiers » sans mesurer invite le `dev-contrat` à inventer du travail pour la remplir.
- **Le critère « sans avertissement neuf » n'était tenu par aucun test.** Seule la fixture minimale était assertée sans avertissement ; la référence ne l'était que par un garde qui ne regarde que `ok`. Refermé par un test dont le message d'échec **nomme** l'avertissement (`texte-trop-long → canon.mj — Canon (MJ)`) au lieu de le compter.
- **Scission de `bug_history.json`** au franchissement du plafond — **4ᵉ axe, né ici : la feature dormante.** BUG-054 (`play-mode`, repointée au Temps 2) et BUG-062 (`global-config`) partent à l'archive sous un critère unifié, « aucun travail en cours ne le relira » — le seul qui décide vraiment de la lecture obligatoire. Garder un défaut de moteur de combat en lecture à chaque session, pendant six itérations d'éditeur, coûtait sans que personne puisse agir dessus. 12 124 o → **7 852 o**.
- **BUG-066 — un refus s'effaçait quand on cliquait « + Ajouter ».** `handleAjouter` indexait son écriture sur l'identifiant **tout juste frappé**, qui n'entre jamais dans le document quand elle échoue : le bandeau devenait inaffichable _et_ évinçait le refus légitime en cours. L'indexation d'un refus avait été pensée pour les chemins d'**édition**, où l'entité existe déjà, puis appliquée telle quelle au chemin de **création**, où elle n'existe pas encore. Trouvé par la revue de PR du `tech-lead`.
- **BUG-067 — la même classe vivait encore dans `dossier-canon`, et elle y est fermée dans le même lot.** `PanneauLieux.tsx` et `ObjectifsCanon.tsx` ne traitaient que `'refuse'` : sur `'absent'`, l'écriture tombait dans la **branche succès** — dossier disparu, aucun message, et un refus en cours effacé par ce faux succès. Une union à trois branches dont une n'est pas nommée se comporte comme sa voisine, en silence. **Élargissement délibéré, hors du plan** : fermer une moitié de classe et laisser l'autre est pire que rien, ça donne l'illusion du traitement. Honnêteté de mesure à consigner : la première version du test d'`ObjectifsCanon` **restait verte sous mutation**, et l'ouvrier l'a dit au lieu de la déclarer prouvée — sur une carte plate, le message d'absence est identique quel que soit l'objectif tagué ; réécrit pour prouver le vrai symptôme.
- ⚠ **Dette structurelle pour it3** : `PanneauPersonnages.tsx` passe de 396 à **443 lignes** et **franchit** le seuil de signal KR-112 (400). Laissé tel quel — 400 est un signal, 800 le bloqueur, et `ObjectifsCanon.tsx` était déjà à 413 avant ce lot. L'extraction d'un hook `useEcriturePersonnage` (brouillon + commit + refus) doit être **actée au raffinage d'it3** comme un lot déclaré, pas improvisée par un ouvrier à court de place.
- ⚠ **`bug_history.json` : la marge rendue par la scission a été consommée aux deux tiers dans la même itération** (12 124 → 7 852 o), puis **refranchi une seconde fois** par les défauts de la revue de PR et ramené sous le plafond par un resserrement d'écriture — le récit part à la revue, le journal garde symptôme, cause, correctif, règle et tests nommés. Mesure finale : **10 234 o pour 10 240, six octets de marge**. it3 le franchit à sa première entrée : sa scission se décide à son raffinage.
- 65 suites / 903 tests. Score de mutation sans objet (aucun des 4 fichiers mutés touché).
...
- **BUG-064** (signalé par l'usage, après livraison) : aucun des 9 tests du panneau n'assertait une valeur **venue du document** — ceux qui touchent le bloc 1 cliquaient avant de vérifier, les autres assertaient un rendu sans donnée. Ils prouvaient que l'écriture atteint le document, jamais que le document revient à l'écran, ni que la fiche suit la ligne **sélectionnée**. Aucun changement de code (le code était juste) ; un test de lecture au montage **sur deux personnages opposés** le couvre désormais, discriminant sur **sept** mutations. Le critère de choix des valeurs semées n'est pas « la deuxième du registre » mais **une valeur que le composant ne peut pas fabriquer sans lire le document** — promu en 11ᵉ critère d'acceptation, à honorer par chaque bloc d'accordéon d'it2 à it5.
- 65 suites / 888 tests. Score de mutation sans objet (aucun des 4 fichiers mutés touché).
...
- **`Lieu` extends `Entite`** (`description?`, `ambiance?`, `dangers?`, tous optionnels — `Monde.lieux: Entite[]` devient `Lieu[]`, extension covariante, zéro régression sur `PanneauDepart`). Les 3 champs sont destination `'ia'` dans `destinations.ts` : ce sont des données de jeu que le futur narrateur (Temps 2) lira, pas de simples notes de rédaction.
- **`PanneauLieux`** (+ `FicheLieu`, extrait au-delà du signal KR-112) : premier consommateur en écriture de `ListRow`, layout liste-à-gauche/fiche-à-droite. Retrait **immédiat, sans `Modal`** — le seul cas dangereux (retirer le lieu de `charpente.depart.lieu_id`) est déjà bloqué par le validateur existant (`reference-pendante`), rendu par le bandeau Refus déjà câblé, sans code neuf.
- **BUG-061** (trouvé en revue de PR, deux tours) : le bandeau de refus n'était pas indexé par lieu — il pouvait rester affiché sous la fiche d'un lieu qui n'avait rien vu refuser après un changement de sélection, ou disparaître sur un succès touchant un _autre_ lieu. Corrigé des deux côtés (affichage et invalidation), même famille que BUG-056 (it1).
- 63 suites / 861 tests. Score de mutation sans objet (aucun des 4 fichiers mutés touché).
- La feature `dossier-canon` (roadmap n° 3) passe **terminée** (4/4).
...
- **Aucun opérande entier**, sur deux motifs indépendants. `docs/REGLES-DU-JEU.md` **ne pose aucune magnitude d'auteur** : le § 1 _calcule_ `PV = FO+AG+EN`, le § 5 _calcule_ tout gain d'XP depuis ΔT, le bonus d'attaque est plafonné par la seule boutique, et le bonus de défense n'apparaît dans aucune section — il vient d'`action-pnj`, feature supprimée. Écrire `gagner_xp: 3` dans le format, ce serait poser une règle de jeu **avant** de l'écrire dans la source de vérité (KR-130). Second motif, indépendant : une magnitude **sans borne nommée** ne peut recevoir aucun test à la limite, donc aucun critère observable (KR-165). Réouverture : doc → table dorée → code, dans cet ordre.
...
- **Sept prédicats, et une propriété que le compilateur tient.** Tout `refKinds[i]` a une ligne dans `COLLECTIONS_IDENTIFIEES`, et `bestiaire` n'en a pas : **aucun prédicat ne peut désigner un monstre, donc aucun ne peut ouvrir un combat**. Mécanique, pas conventionnel. `jet_reussi` est **veto** — un jet n'évalue pas, il _émet_ une demande qui change le tour ; un évaluateur qui en contient lance le dé.
...
- **`validate.ts` : 723 → 607 lignes** _(corrigé au raffinage d'it4 : le « 602 » publié ici était vrai à la mesure, faux après les correctifs de la revue de PR ; et le « plafond 650 » qui l'accompagnait n'existait nulle part — KR-176)_. `tables.ts` sort **avec son bénéficiaire**, la 4ᵉ assertion du balayage — dernier chemin de contournement du garde de `DESTINATION_DES_CHAMPS`, où un champ ajouté aux types et aux tables mais pas à la fixture restait invisible.
- **BUG-051, trouvé par la QA et corrigé avant la revue humaine** : les quatre lignes `…_texte` neuves n'étaient tenues par **aucun test de valeur** — les basculer en `ia` laissait 168 tests verts. Le trou était **dans le plan**, pas dans le code (KR-174 : une assertion d'_existence_ n'est jamais une assertion de _valeur_). Aussi : BUG-052 (propriété affirmée en docstring sans test) et BUG-050 (élément de liste non-objet, reporté it4, journalisé avant la première ligne de code).
...
- **`docs/WORKFLOW.md`** — _Architecture Vocabulary_ supprimée (un tableau **vide** sous une consigne d'usage). _Hover-reveal row actions_ : la règle garde sa valeur, son exemple était en MUI `sx` / `IconButton` — il n'y a pas de MUI dans ce dépôt, l'exemple apprenait une stack qui n'existe pas. _Design Patch Processing_ condensée en deux phrases : procédure dormante (`genliv_changes/` n'existe pas), elle garde sa capacité et perd sa cérémonie.
