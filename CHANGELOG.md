...
- **`validate.ts` : 723 → 602 lignes.** `tables.ts` sort **avec son bénéficiaire**, la 4ᵉ assertion du balayage — dernier chemin de contournement du garde de `DESTINATION_DES_CHAMPS`, où un champ ajouté aux types et aux tables mais pas à la fixture restait invisible.
...
- **48 suites / 662 tests** (47 / 603 avant). Budget de contexte : `specification.json` a franchi son plafond au report et a été **compacté dans le même geste** — 65 kio tenus, marge 209 o.
