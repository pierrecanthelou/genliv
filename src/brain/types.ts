/**
 * Domain model — les types de RÈGLES du jeu (objets, jets, monstres, pièges,
 * décors, PNJ) et l'état de synchronisation. Références par identifiant stable,
 * jamais par nom.
 *
 * La STRUCTURE D'ARBRE d'un livre (`Book`, `BookNode`, `Edge`, `ChoicePrereq`,
 * `ChoiceCountdown`, `NodeActionType`) vit désormais dans `./tree` — scission
 * opérée par la feature n° 1 `dossier-format`. Le sens de la dépendance est à
 * SENS UNIQUE : `tree.ts` importe d'ici, jamais l'inverse. Ce qui reste ici est
 * la couche conservée, consommée par `src/player/` jusqu'à la n° 9 ; l'arbre est
 * la couche condamnée.
 */
