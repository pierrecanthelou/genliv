...
import { CARD1_TITRE } from '../textes'

/**
 * L'ACCEPTATION DE LA CARTE 1 — critère 4 (it1) / critère 7 (it2) du plan :
 * l'écriture passe UNIQUEMENT par `DossierService.update(id, recette)`, dans
 * l'ordre persistance-puis-événement (KR-004) ; un refus de `validateDossier`
 * ne persiste rien, n'émet rien, et rend les anomalies — cas NOMINAL, pas une
 * branche d'erreur (KR-234). Et les deux variantes de `LigneProposition`.
 *
 * Fichier SÉPARÉ de `panneauCopilote.test.tsx` et de `detenteurs.test.tsx`
 * (§ 5 du plan) — mêmes petits helpers de montage/seed, DUPLIQUÉS ici à
 * dessein (précédent `fichePersonnage.test.tsx`/`panneauPersonnages.test.tsx`,
 * dossier-fiches).
 *
 * DEPUIS L'ITÉRATION 2, un second bouton « Lancer » (carte 2) coexiste à
 * l'écran : toutes les requêtes sont cadrées `within(regionCarte1())`, le
 * point d'ancrage `<section aria-label={CARD1_TITRE}>` exposé par
 * `CarteAssistant`.
 */
...
describe('acceptation - ecriture par update seul', () => {
	it('accepter ecrit via DossierService.update, ordre persistance puis evenement, un seul appel', async () => {
		...
		await within(regionCarte1()).findByRole('button', { name: 'Accepter la proposition' })
		...
		fireEvent.click(within(regionCarte1()).getByRole('button', { name: 'Accepter la proposition' }))
		...
		expect(await within(regionCarte1()).findByText('Accepté')).toBeInTheDocument()
	})
})
...
describe('acceptation - refus de validateDossier, cas nominal', () => {
	it('rien n est persiste, aucun evenement, les anomalies sont rendues, la proposition reste affichee', async () => {
		...
		expect(within(regionCarte1()).getByText('Anomalie de test — refus simule.')).toBeInTheDocument()
		// Cas NOMINAL (KR-234), pas une branche d'erreur : la proposition reste
		// affichée, l'auteur peut toujours l'accepter ou la rejeter.
		expect(within(regionCarte1()).getByRole('button', { name: 'Accepter la proposition' })).toBeInTheDocument()
		expect(within(regionCarte1()).getByRole('button', { name: 'Rejeter la proposition' })).toBeInTheDocument()
	})
})
...
describe('acceptation - les deux variantes de LigneProposition', () => {
	it('cible vide : REMPLISSAGE, un seul Field, aucun bloc AVANT', async () => {
		...
		await within(regionCarte1()).findByRole('button', { name: 'Accepter la proposition' })
		expect(within(regionCarte1()).queryByText('AVANT')).toBeNull()
		expect(within(regionCarte1()).getByRole('textbox', { name: /^FONCTION/ })).toHaveValue('Gardien du seuil.')
	})
	...
	it('cible redigee : REMPLACEMENT, bloc AVANT visible puis le meme Field APRES', async () => {
		...
		await within(regionCarte1()).findByRole('button', { name: 'Accepter la proposition' })
		expect(within(regionCarte1()).getByText('AVANT')).toBeInTheDocument()
		expect(within(regionCarte1()).getByText('Ancienne fonction.')).toBeInTheDocument()
		expect(within(regionCarte1()).getByRole('textbox', { name: /^FONCTION/ })).toHaveValue('Nouvelle fonction.')
	})
})
 ...
 * de l'écran au moment précis où il est écrasé (BUG-097). `jest` reste vert, et
 * le seul instrument anti-complaisance de l'itération devient un diff de A
 * contre A.
 */
