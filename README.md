...
> L'écran d'édition **de livre** (`EditorScreen.tsx`, route `editor`) est une **coquille intacte et injoignable** : barre supérieure + canevas d'arbre, sans panneau d'édition, plus aucun chemin d'IHM n'y mène depuis `bascule-editeur` it1 — sa démolition physique est reportée à n° 9. L'écran d'édition **de dossier** (`DossierEditorScreen`, route `dossier`, fichier neuf de `bascule-editeur`) affiche depuis it3 sa vraie navigation à deux colonnes : dix `ListRow` (Canon, Départ, Personnages, Lieux, Objets, Indices, Quêtes, Événements, Conditions, Jalons & fins) avec compteur exact à gauche. **Cinq** des dix sections ont désormais un vrai formulaire : Canon (`PanneauCanon` + `ObjectifsCanon`), Départ (`PanneauDepart`) et Lieux (`PanneauLieux` + `FicheLieu`) depuis `dossier-canon` it1→it4, Personnages (`PanneauPersonnages` + `FichePersonnage`, terminée 8/8) depuis `dossier-fiches`, et Objets (`PanneauObjets` + `FicheObjet`, terminée 2/2) depuis `dossier-objets` it1→it2 — tous injectés via le slot `panneaux` de `App.tsx`, jamais un import direct entre features ; les cinq autres restent à état vide nommé, en attente de leurs propres itérations (n° 6).
