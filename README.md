...
| `book-library` | skeleton ✅ | Home screen `LibraryScreen`: grid of book cards (live VIEW via brain `useBooks`), newest first; click → open + navigate; ✕ → confirmation dialog → `BookService.deleteBook` (persist-remove → `book:deleted`). Composes `book-creation`'s create affordance as a prop (KR-072). Rename/search/sort + currently-open-book guard deferred. |
...
| `outline-view` | skeleton ✅ | The open book as an indented « plan » (`OutlineView`): DFS from the sommaire nesting `choice` edges, `relink`/`flee`/cycles shown as `↪` references (cycle-safe), isolated nodes flat. Shared editor `EditorTopBar` (brain) with a canvas ↔ outline switch; `EditorScreen` shell swaps the body; selection shared via `SelectionService`. View-mode persistence deferred until `UIPreferencesService`. |
| `action-decor` | skeleton ✅ | First `action-*` feature: self-registers a « Décor » editor with the brain `ActionRegistry` (node-editor mounts it without importing it). Prendre / Écouter / Fouiller switch persisted on `node.decor`; « prendre » uses the new shared **`brain/components/ObjectEditor`** (name interne + description joueur, KR-052) with a stable object id. Multi-object + rolls + écouter/fouiller + shared `ObjectCatalogService` deferred to iterations. |
...
**🏁 The `0.1.x` MVP tier is complete** — every feature has a walking skeleton. We build **breadth-first** (see `docs/ROADMAP.md`): `0.1.x` = MVP, `0.2.x` = V1 (iter 1 of each, in build order), etc. Next: `0.2.x` — `book-creation` iteration 1.
