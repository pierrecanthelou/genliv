...
**The `0.2.x` / V1 tier is complete** — every feature has its iteration 1. We build **breadth-first** (see `docs/ROADMAP.md`): `0.1.x` = MVP, `0.2.x` = V1, `0.3.x` = V2, etc. **🏁 Tier `0.3.x` / V2 is now open** — iteration 2 of each feature, in build order: `choice-linking` iter 2 shipped (relink popover + self-link/duplicate guards); next up are the remaining V2 iterations (`book-library`, `outline-view`, `action-*`, `cloud-sync`; `tree-canvas` iter-2 banked depth-first → skipped).
