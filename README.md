...
**🏁 The `0.2.x` / V1 tier is complete** — every feature has its iteration 1 (`choice-linking`, `book-library`, `outline-view`, `action-decor`, `action-pnj`, `action-monster`, `action-trap`, `cloud-sync`; `book-creation`/`tree-canvas`/`node-editor` iter-1 were banked depth-first). We build **breadth-first** (see `docs/ROADMAP.md`): `0.1.x` = MVP, `0.2.x` = V1, `0.3.x` = V2, etc. Next tier: `0.3.x` / V2 — iteration 2 of each feature, in build order, starting with `choice-linking` iteration 2 (relink popover + self-link/duplicate guards).
