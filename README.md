...
src/
  ...
  features/         # isolated feature modules; talk to the rest only through brain/
    ...
    dossier-objets/     # n° 5 — 1/2: PanneauObjets + FicheObjet, registre d'objets (nom + description_joueur, sans borne), reorder via boutons Monter/Descendre composes par la feature
...
**Deux features neuves de plus se sont achevées** — n° 3 (le canon et ses annexes) et n° 4 (la fiche personnage) :
...
**Une feature neuve est en cours** — n° 5, le registre des objets :

| Feature | État | Résumé |
| --- | --- | --- |
| `dossier-objets` | **1/2 — en cours** — registre ✅ · retrait ⏳ | it1 pose `Objet extends Entite { description_joueur?: string }` (`ia`, sans borne — même famille que `Lieu.description`/`Personnage.description_joueur`) et remplace l'état vide de la section Objets par `PanneauObjets` + `FicheObjet` (liste-à-gauche/fiche-à-droite, précédent `PanneauLieux`/`FicheLieu`). Le réordonnancement, présupposé au cadrage comme une poignée de glisser câblée sur `ListRow.onReorder`, a été écarté au raffinage par les 4 rôles (aucun équivalent clavier, non prouvable en jsdom, anti-patron KR-109) : deux boutons Monter/Descendre, composés **entièrement par la feature**, `ListRow.tsx` restant inchangé en code (docstring seul). it2 ajoutera le retrait, refusé au SSOT si l'objet est encore référencé par une contrepartie de savoir. |
...
> L'écran d'édition **de livre** (`EditorScreen.tsx`, route `editor`) est une **coquille intacte et injoignable** : barre supérieure + canevas d'arbre, sans panneau d'édition, plus aucun chemin d'IHM n'y mène depuis `bascule-editeur` it1 — sa démolition physique est reportée à n° 9. L'écran d'édition **de dossier** (`DossierEditorScreen`, route `dossier`, fichier neuf de `bascule-editeur`) affiche depuis it3 sa vraie navigation à deux colonnes : dix `ListRow` (Canon, Départ, Personnages, Lieux, Objets, Indices, Quêtes, Événements, Conditions, Jalons & fins) avec compteur exact à gauche. **Cinq** des dix sections ont désormais un vrai formulaire : Canon (`PanneauCanon` + `ObjectifsCanon`), Départ (`PanneauDepart`) et Lieux (`PanneauLieux` + `FicheLieu`) depuis `dossier-canon` it1→it4, Personnages (`PanneauPersonnages` + `FichePersonnage`, terminée 8/8) depuis `dossier-fiches`, et Objets (`PanneauObjets` + `FicheObjet`, 1/2) depuis `dossier-objets` it1 — tous injectés via le slot `panneaux` de `App.tsx`, jamais un import direct entre features ; les cinq autres restent à état vide nommé, en attente de leurs propres itérations (n° 5 suite et n° 6).
