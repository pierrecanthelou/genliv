...
src/
  ...
  features/         # isolated feature modules; talk to the rest only through brain/
    ...
    bascule-editeur/    # n° 2 — 2/3: DossierEditorScreen (minimal) lives here; nav sections in it3
...
| `bascule-editeur` | **2/3** | La navigation latérale passe de l'arbre à une liste de sections. it1 repointe `book-library` sur `DossierService` (`list`/`remove`, `DossierResume` en union discriminée lisible/illisible) : la bibliothèque liste, télécharge et supprime des dossiers d'aventure, BUG-048 (dossier-format) fermé pour de bon par un constat de présence par clé. `tree-canvas` mis en sommeil (repointage réel reporté à n° 6, faute de données relationnelles avant n° 4/5/6). it2 repointe `book-creation` sur `Dossier` (`DossierService.create`, seed marqué `⟨à écrire⟩` jamais joué tel quel), fait naître `DossierEditorScreen` (écran minimal, nouveau fichier — `EditorScreen.tsx` reste intact) sur une nouvelle route `{name:'dossier'}`, et rend une carte de bibliothèque cliquable pour rouvrir un dossier déjà présent. it3 livrera la vraie liste de sections. |
...
> L'écran d'édition **de livre** (`EditorScreen.tsx`, route `editor`) est une **coquille intacte et injoignable** : barre supérieure + canevas d'arbre, sans panneau d'édition, plus aucun chemin d'IHM n'y mène depuis `bascule-editeur` it1 — sa démolition physique est reportée à n° 9. L'écran d'édition **de dossier** (`DossierEditorScreen`, route `dossier`, fichier neuf de `bascule-editeur`, livré it2) est lui aussi une coquille pour l'instant : barre supérieure + état vide nommé, sans navigation — la liste de sections du dossier la remplace en n° 3 (itération 3 de `bascule-editeur`).
