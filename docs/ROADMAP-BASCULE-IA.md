...
| 7 | `dossier-controles` | …voir pourquoi son aventure n'est pas encore jouable | 6 | 5/6 | 5 rôles | 6 |
...
**7 · `dossier-controles`** — le linter d'aventure : un rapport de contrôles, un panneau, des badges par section. Nommée `controles` et non `lint` : dans ce dépôt `npm run lint` désigne ESLint depuis l'itération outillage-1. **6 itérations, 10 règles** (5 bloquantes, 4 alertes, 1 info). Aucun lot contrat sur la triade de la Décision A. Historique des recadrages (3 → 4 au cadrage, 4 → 6 au raffinage d'it3), mesures et arbitrages : `src/features/dossier-controles/specification.json` et `.claude/raffinage/dossier-controles-it*.revue.md`.
