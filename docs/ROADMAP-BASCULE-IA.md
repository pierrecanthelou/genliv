...
|---|---|---|---|
| `tree-canvas` | 1 703 | **en sommeil ; repointage NON ASSIGNÉ** | Décision n° 5 : même composant, autre source, le graphe passe de nœuds + arêtes à relations + indices — mais AUCUNE feature ne le porte plus. CORRECTION (cadrage `bascule-editeur`, 2026-08-08) : déplacée de n° 2 à n° 6 — mesuré au cadrage, `Monde.lieux/objets/indices` sont `Entite={id, nom?}` nus et `Personnage` n'a pas de `relations[]` avant n° 4 ; n° 6 semblait la première feature où les trois sources du graphe (relations n° 4, `lieux[].acces` n° 5, indices + chaînage n° 6) existent. **CORRECTION (cadrage `dossier-registres`, 2026-08-17)** : repointage visuel retiré du périmètre de n° 6 — `tree-canvas` reste câblé sur `BookNode`/`Edge` condamnés, sa démolition est déjà planifiée en n° 9, et aucun instrument ne couvre pan/zoom/layout. `lieux[].acces` reste sans propriétaire. `tree-canvas` reste sur disque, intact (« en sommeil »), sans repointage assigné. Détail : `src/features/dossier-registres/specification.json` (KR-204/205, `open_questions`). |
...
| 6 | `dossier-registres` | …tenir les quêtes, les indices, les événements de son aventure | 5 | **2/5** | 4 rôles | 4 · 5 |
