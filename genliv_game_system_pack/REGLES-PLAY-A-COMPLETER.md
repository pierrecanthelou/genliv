...
**A2. Affichage d'un écran.** À chaque nœud : afficher `text`, puis résoudre l'`actionType` requis (décor / pnj / piège / monstre) **avant** de proposer les choix sortants. ✍️ *Défaut : OK — l'action requise se résout d'abord ; les choix n'apparaissent qu'ensuite.*
...
**B1. Génération.** 2D4 par carac + 1D4 réparti sur 3 caracs choisies (§ 1). ✍️ *Défaut : OK.*
...
**B3. Équipement de départ.** Le héros commence-t-il avec une arme/armure ? ✍️ *Défaut proposé : mains nues (×0.3), aucune protection, 0 objet — tout vient du livre.*
...
**D3. IA du monstre (choix de posture).** Comment le monstre choisit sa posture chaque round ?
  ✍️ *Défaut proposé : pondéré — 60 % Normale, 25 % Précise, 15 % Défensive ; passe à 40 % Défensive si ses PV < 25 %.* (À ajuster, ou rendre dépendant de l'IG du monstre.)

**D4. Coût d'endurance.** -1 PE par round pour le héros (§ 1). Le monstre a-t-il une jauge PE ? ✍️ *Défaut proposé : non — seul le héros gère le PE (sauf capacité « pas d'endurance » du Squelette, qui devient sans objet).* 
...
**D8. Dégradation d'armure (coup critique).** Un critique retire 1 pt d'armure **de la cible** pour le reste du combat (§ 3). S'applique au héros ET au monstre. ✍️ *Défaut : OK.*
...
**E1. État « inconscient » (0 ≥ PV > −CA).** Que se passe-t-il ?
  ✍️ *Défaut proposé : en combat, inconscient = le monstre porte un dernier assaut ; si PV repasse ≤ −CA → mort, sinon le héros se réveille à 1 PV à la fin du combat. Hors combat, inconscient est traité comme mort (game over).* (À trancher — c'est important.)
...
**G2. Anti-farm.** Un monstre/objet déjà résolu sur un écran se **re-déclenche** s'il y revient, ou reste consommé ?
  ✍️ *Défaut proposé : consommé — un objet ramassé ne se reprend pas, un monstre vaincu ne se recombat pas (mémorisé par id de nœud).*
...
| Squelette | passif | ignore le PE (n'a pas de jauge) — n'impacte que lui ✍️ *décor possible* |
| Zombie | atteint 0 PV | jet 1D6 : sur 5–6, repasse à 1 PV (une seule fois ?) ✍️ |
...
| Méduse | — | l'auteur **choisit** à la création : soit -3 AT permanent (aveugle), soit risque pétrification (mort sur critique). À exposer comme option éditeur ? ✍️ |
| Spectre | passif | n'encaisse **qu'1 dégât max** par coup d'arme non-magique ✍️ *(notion d'arme magique à définir)* |
| Ogre | gagne, même si héros pare | détruit 1 pt d'armure/bouclier du héros ✍️ |
| Momie | passif + critique | immunise les postures **Précises** (×1 au lieu de ×2) ; critique = soins bloqués ✍️ |
| Manticore | début du combat | 1D3 piques avant le corps-à-corps (AT fixe 5, 3 dégâts chacune) ✍️ |
| Troll | chaque round | régénère 3 PV ; annulé le round où le héros inflige feu/acide ✍️ *(source feu/acide à définir)* |
...
| Liche | passif + critique | ignore l'armure du héros (magie) ; critique = draine 1D4 PV **max** ✍️ |
| Tyrannœil | chaque round | rayon 1D4 : 1=+2 dégâts, 2=-2 AT héros, 3=-2 EN, 4=mort si Écart > 5 ✍️ |
...
- **Arme magique / arme en argent** : un objet a-t-il un attribut « magique » / « argent » (Spectre, Loup-Garou) ? ✍️ *Défaut proposé : ajouter un flag optionnel sur l'`EquipmentEffect` (`magic?`, `silver?`).*
- **Source feu/acide** (Troll) : une arme/objet peut-il porter un type de dégât ? ✍️ *Défaut proposé : flag optionnel `damageType?: 'feu' | 'acide'` sur l'arme.*
- **« Objet mineur »** (Gobelin) : défini comme objet sans effet d'équipement. ✍️
